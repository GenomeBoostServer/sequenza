read.seqz <- function(file, n_lines = NULL, col_types = "ciciidddcddccc", chr_name = NULL,
    buffer = 33554432, parallel = 1, col_names = c("chromosome", "position", "base.ref",
    ...
    if (tbi) {
        ...
        read.seqz.chr(file, chr_name = chr_name, col_types = col_types, col_names = col_names,
...
read.seqz.chr <- function(file, chr_name, col_names, col_types, skip, buffer, parallel) {
    ...
    read_tsv_chunked(file, DataFrameCallback$new(f), col_types = col_types, skip = skip,
        col_names = col_names)
}
