...
read.seqz <- function(file, n_lines = NULL, col_types = "ciciidddcddccc",
    chr_name = NULL, buffer = 33554432, parallel = 1, col_names = c("chromosome",
        "position", "base.ref", "depth.normal", "depth.tumor",
    ...
    if (cache) {
        cache_key <- digest::digest(list(file, n_lines, col_types,
            chr_name, buffer))
    ...
    range_info <- if (!is.null(n_lines)) {
        ...
        list(skip = n_lines[1], n_max = diff(round(sort(n_lines))) +
            1)
    } else {
    ...
    result <- if (file.exists(paste(file, "tbi", sep = "."))) {
        ...
        read.seqz.chr(file, chr_name = chr_name, col_types = col_types,
            col_names = col_names, skip = range_info$skip, buffer = buffer,
            parallel = parallel)
    }
...
read.seqz.chr <- function(file, chr_name, col_names, col_types,
    ...
    parse_chunk <- function(x, chr_name, col_names, col_types) {
        ...
        chunk_data <- read_tsv(file = chunk_text, col_types = col_types,
            col_names = col_names, skip = 0, n_max = Inf, progress = FALSE,
            show_col_types = FALSE, lazy = TRUE  # Enable lazy reading
)

        # Efficient chromosome filtering using
        # data.table-style optimization
        if (!is.null(chr_name)) {
    ...
    results <- chunk.apply(input = con, FUN = parse_chunk, chr_name = chr_name,
...
read.seqz.tbi <- function(file, chr_name, col_names) {
    ...
    tabix_range <- if (!is.null(chr_name)) {
        if (grepl(":", chr_name)) {
            ...
            if (length(parts) != 3) {
                stop("Invalid coordinate format. Expected 'chr:start-end', got: ",
                  chr_name)
            }
            chr_name  # Keep original format if it includes coordinates
        } else {
            # For chromosome-only queries, add full range
            # specification
    ...
    res <- tabix.read.table(file, tabix_range, col.names = TRUE,
