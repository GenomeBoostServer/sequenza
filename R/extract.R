# file = data.file; window = 1e6; overlap = 1; slide_win =
# 100; peak_wins = seq(from = 50, to = 300, by = 25);
# mufreq.treshold = 0.10; min.reads = 40; min.reads.normal
# = 10; min.reads.baf = 1; max.mut.types = 1; min.type.freq
# = 0.9; min.fw.freq = 0; verbose = TRUE; chromosome.list =
# NULL; breaks = NULL; assembly = 'hg19'; weighted.mean =
# TRUE; normalization.method = 'mean'; ignore.normal =
# FALSE; parallel = 1; gc.stats = NULL; segments.samples =
# FALSE

sequenza.extract <- function(
    file, window = 1e+06, overlap = 1, slide_win = 100, peak_wins = seq(from = 50, to = 300, by = 25),
    mufreq.treshold = 0.1, min.reads = 40, min.reads.normal = 10,
    min.reads.baf = 1, max.mut.types = 1, min.type.freq = 0.9,
    min.fw.freq = 0, verbose = TRUE, chromosome.list = NULL,
    breaks = NULL, assembly = "hg38", weighted.mean = TRUE, normalization.method = "mean",
    ignore.normal = FALSE, parallel = 1, gc.stats = NULL, segments.samples = FALSE,
    ...
    if (is.null(gc.stats)) {
        gc.stats <- gc.sample.stats(
            file, verbose = verbose, parallel = parallel, smooth = smooth_gc,
    ...
    if (do_raster) {
        ratio_baf_raster <- baf_ratio_raster(
            file_name = file, gc_normal = gc_spline_normal, gc_tumor = gc_spline_tumor,
            verbose = verbose, min_times = min_times_gc, smooth = do_raster,
            grid_size = gc_grid, scale.subset = 2.5, round_baf = 2,
            round_ratio = 1
        )
    } else {
    ...
    if (is.null(chromosome.list)) {
        ## NOTE: instead using all the chromosomes in the
        ## seqz we download the assembly from goldenpath
        ## -or use a cached verion (TODO)- and use only the
        ## chromosomes with a centromere.  A special
        ## attentions need to be taken to the 'chr' previx
        ## The workflow need also to be optimized since we
        ## will use the same resource later, it could be
        ## fetched once and used also downstream
    ...
    for (chr in chromosome.list) {
        ...
        seqz.data <- read.seqz(
            file, n_lines = c(file.lines$start, file.lines$end),
            chr_name = chr
        )
        ...
        if (ignore.normal) {
            ...
            seqz.data$adjusted.ratio <- round(norm_tumor_depth/norm_normal_depth, 3)
        }
        if (segments.samples == TRUE) {
            breaks_normal_chr <- NULL
            breaks_tumor_chr <- NULL
            # breaks_normal_chr <- breaks_full( data =
            # data.frame(chromosome = seqz.data$chromosome,
            # position = seqz.data$position, adjusted.ratio
            # = norm_normal_depth, singsAsFactors = FALSE),
            # gamma = gamma.pcf, kmin = kmin.pcf, assembly
            # = assembly, breaks.het = NULL)
            # breaks_tumor_chr <- breaks_full( data =
            # data.frame(chromosome = seqz.data$chromosome,
            # position = seqz.data$position, adjusted.ratio
            # = norm_tumor_depth, singsAsFactors = FALSE),
            # gamma = gamma.pcf, kmin = kmin.pcf, assembly
            # = assembly, breaks.het = NULL)
        } else {
            breaks_normal_chr <- NULL
        ...
        seqz.n.win <- windowValues(
            x = seqz.data$depth.normal/avg_nor_depth, positions = seqz.data$position,
            chromosomes = seqz.data$chromosome, window = window,
            overlap = overlap
        )
        seqz.t.win <- windowValues(
            x = seqz.data$depth.tumor/avg_tum_depth, positions = seqz.data$position,
            chromosomes = seqz.data$chromosome, window = window,
            overlap = overlap
        )
        seqz.r_r.win <- windowValues(
            x = seqz.data$depth.ratio/(avg_tum_depth/avg_nor_depth),
            positions = seqz.data$position, chromosomes = seqz.data$chromosome,
            window = window, overlap = overlap, weight = seqz.data$depth.normal
        )
        seqz.n_n.win <- windowValues(
            x = norm_normal_depth, positions = seqz.data$position,
            chromosomes = seqz.data$chromosome, window = window,
            overlap = overlap
        )
        seqz.n_t.win <- windowValues(
            x = norm_tumor_depth, positions = seqz.data$position,
            chromosomes = seqz.data$chromosome, window = window,
            overlap = overlap
        )
        ...
        mut.tab <- mutation.table(
            ...
            min.type.freq = min.type.freq, min.fw.freq = min.fw.freq,
            segments = seg.s1
        )
        ...
        segments_samples.list[[which(chromosome.list == chr)]] <- list(normal = breaks_normal_chr, tumor = breaks_tumor_chr)
        rank_peaks.list[[which(chromosome.list == chr)]] <- list(selected_win = select_win, peak_win = compare_bins_segs)

        if (verbose) {
            message(
                "   ", nrow(mut.tab),
                " variant calls.", appendLF = TRUE
            )
            message(
                "   ", nrow(seg.s1),
                " copy-number segments.", appendLF = TRUE
            )
            message(
                "   ", nrow(seqz.het),
                " heterozygous positions.", appendLF = TRUE
            )
            message(
                "   ", sum(seqz.hom),
                " homozygous positions.", appendLF = TRUE
            )
        }
    }
    ...
    gc_norm <- unfold_gc(
        cbind(unique = 0, lines = 0, do.call(rbind, norm.gc.list)),
        stats = FALSE, smooth = smooth_gc, min_times = min_times_gc,
    ...
    list(
        BAF = windows.baf, ratio = windows.ratio, raw_ratio = windows.raw_ratio,
        depths = list(
        ...
        chromosomes = chromosome.list, gc = gc.stats, gc_norm = gc_norm,
        avg.depth.ratio = avg_depth_ratio, avg.depth.tumor = avg_tum_depth,
        avg.depth.normal = avg_nor_depth, segments_samples = segments_samples.list,
        win_peaks = rank_peaks.list, ratio_baf_raster = ratio_baf_raster$dr
    )
}
