...
process_depths <- function(seqz.data, gc_splines, avg_depths, ignore.normal) {
    ...
    norm_gc_stats <- depths_gc(depth_n = round(normal_depth * avg_depths$normal, 0),
...
calculate_windows <- function(seqz.data, depths, window, overlap, avg_depths) {
    ...
    required_cols <- c("adjusted.ratio", "depth.ratio", "position", "chromosome", "depth.normal")
    if (!all(required_cols %in% names(seqz.data))) {
        stop("Missing required columns in seqz.data: ",
             paste(setdiff(required_cols, names(seqz.data)), collapse = ", "))
    }

    # Calculate all window values at once
    list(
        ratio = windowValues(x = seqz.data$adjusted.ratio, positions = seqz.data$position,
        ...
        normal = windowValues(x = seqz.data$depth.normal/avg_depths$normal, 
            ...
            chromosomes = seqz.data$chromosome, window = window, overlap = overlap),
        tumor = windowValues(x = seqz.data$depth.tumor/avg_depths$tumor, 
            positions = seqz.data$position,
            chromosomes = seqz.data$chromosome, window = window, overlap = overlap),
        ...
        n_normal = windowValues(x = depths$normal, positions = seqz.data$position, 
            chromosomes = seqz.data$chromosome, window = window, overlap = overlap),
        n_tumor = windowValues(x = depths$tumor, positions = seqz.data$position,
            chromosomes = seqz.data$chromosome, window = window, overlap = overlap),
        baf = list()
    )
}
...
initialize_extract_containers <- function(chromosome.list) {
    ...
    containers <- list(
        windows.baf = vector("list", n_chr),
        windows.ratio = vector("list", n_chr),
        windows.raw_ratio = vector("list", n_chr),
        windows.normal = vector("list", n_chr),
        windows.tumor = vector("list", n_chr),
        windows.n_normal = vector("list", n_chr),
        windows.n_tumor = vector("list", n_chr),
        mutation.list = vector("list", n_chr),
        segments.list = vector("list", n_chr),
        norm.gc.list = vector("list", n_chr),
...
process_single_chromosome <- function(chr, file, gc_stats, gc_splines, containers,
    ...
    # Store results
    containers <- store_chromosome_results(list(windows = windows, segments = segments, mutations = mutations,
...
#' @title Extract Sequenza Data
#' @description Process and extract sequencing data for copy number analysis
#' @param file Input file path
#' @param window Window size for analysis
#' @param overlap Overlap between windows
#' @param ... Additional parameters
#' @return List containing processed sequencing data
#' @export
sequenza.extract <- function(file, window = 1e+06, overlap = 1, slide_win = 100,
    ...
    # Initialize logging
    log_file <- paste0(tempdir(), "/sequenza_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".log")
    ...
    tryCatch({
        ...
        if (params$parallel > 1) {
                ...
                containers <- store_chromosome_results(
                    list(
                        windows = list(
                            ...
                            raw_ratio = results[[idx]]$windows.raw_ratio[[idx]],
                            normal = results[[idx]]$windows.normal[[idx]],
                            tumor = results[[idx]]$windows.tumor[[idx]],
                            n_normal = results[[idx]]$windows.n_normal[[idx]],
                            n_tumor = results[[idx]]$windows.n_tumor[[idx]],
                            baf = results[[idx]]$windows.baf[[idx]]
                        ),
                        segments = list(seg = results[[idx]]$segments.list[[idx]]),
                        mutations = results[[idx]]$mutation.list[[idx]],
            ...
            for (chr in params$chromosome.list) {
                containers <- process_single_chromosome(chr, file, params$gc.stats, gc_splines,
                    containers, params)
            }
        }
...
initialize_extract_parameters <- function(file, window, overlap = 1, slide_win = 100, peak_wins = seq(from = 50,
    to = 300, by = 25), normalization.method = "mean", ignore.normal = FALSE, verbose = TRUE,
    chromosome.list = NULL, breaks = NULL, min.mut.freq = 0.1, min.reads = 40, min.reads.normal = 10,
    min.reads.baf = 1, max.mut.types = 1, min.type.freq = 0.9, min.fw.freq = 0, assembly = "hg38",
    gc.stats = NULL, do_raster = FALSE, smooth_gc = FALSE, min_times_gc = 5, gc_grid = 250,
    parallel = 1, weighted.mean = TRUE,
    ...) {
    ...
    list(window = window, overlap = overlap, slide_win = slide_win, peak_wins = peak_wins,
        ...
        min.fw.freq = min.fw.freq, gc.stats = local_gc_stats, do_raster = do_raster, smooth_gc = smooth_gc,
        min_times_gc = min_times_gc, gc_grid = gc_grid, parallel = parallel, ratio_baf_raster = data.frame(dr = NULL),
        avg_depths = list(normal = weighted.mean(local_gc_stats$normal$depth, colSums(local_gc_stats$normal$n)),
            tumor = weighted.mean(local_gc_stats$tumor$depth, colSums(local_gc_stats$tumor$n))),
        weighted.mean = weighted.mean)
}
...
finalize_extract_results <- function(containers, params, gc_stats, gc_data) {
    ...
    gc_norm <- unfold_gc(
        cbind(unique = 0, lines = 0, do.call(rbind, containers$norm.gc.list)),
        stats = FALSE,
        smooth = params$smooth_gc,
        min_times = params$min_times_gc,
        cl = params$parallel,
    ...
    list(BAF = containers$windows.baf, ratio = containers$windows.ratio, raw_ratio = containers$windows.raw_ratio,
        ...
        win_peaks = containers$rank_peaks.list,  # Add rank_peaks to output
        chromosomes = params$chromosome.list, gc = gc_stats, gc_norm = gc_norm, avg.depth.ratio = depths$avg_depth_ratio,
...
rank_segments <- function(breaks_list, windows, params) {
    if (length(breaks_list) <= 1) {
        return(list(
            segs = breaks_list[[1]],
            selected_win = params$peak_wins[1],
            peak_win = data.frame(
                peak_win = params$peak_wins[1],
                baf_fit = 0,
                ratio_fit = 0,
    ...
    compare_bins_list <- if (params$parallel > 1) {
        parallel::mclapply(breaks_list, function(x) {
            ...
            ratio_vs_bins <- safely_compare_bins(x$start.pos, x$end.pos, x$depth.ratio, ratio_win)
    ...
    ranks_fits <- cbind(
        ...
        ratio = rank(-compare_bins_segs$ratio_fit, ties.method = "max"),
    ...
    if (params$verbose) {
        ...
        message("Number of segments: ", compare_bins_segs$n_segs[compare_bins_segs$peak_win == select_win])
    }
