mut.fractions <- function(AB.tumor, Af, tumor.strand) {
  ...
  max.fq <- function(x) {
    ...
    c(
      freq.rel[f.max], names(base.freqs[[x]])[f.max], base.freqs[[x]][f.max],
      fw.freqs[[x]][f.max]
    )
  }
  ...
  result <- data.frame(
    base.count = as.integer(n.base.mut), maj.base.freq = as.numeric(max.freqs[
      ,
      1
  ...
  if (nrow(result) != length(AB.tumor)) {
    warning(paste(
      "Mismatch in result rows. Expected:", length(AB.tumor), "Actual:",
      nrow(result)
    ))
  }
...
mutation.table <- function(
  seqz.tab, mufreq.threshold = 0.15, min.reads = 40, min.reads.normal = 10,
  ...
  required_cols <- c(
    "chromosome", "position", "zygosity.normal", "AB.tumor", "good.reads",
