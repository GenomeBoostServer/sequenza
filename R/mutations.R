mut.fractions <- function(AB.tumor, Af, tumor.strand) {
    ...
    result <- data.frame(base.count = as.integer(n.base.mut), maj.base.freq = as.numeric(max.freqs[,
        1]), base = as.character(max.freqs[, 2]), freq = as.numeric(max.freqs[, 3]),
        fw.freq = as.numeric(max.freqs[, 4]))

    # Validate result dataframe
    if (nrow(result) != length(AB.tumor)) {
        warning(paste("Mismatch in result rows. Expected:", length(AB.tumor), "Actual:",
            nrow(result)))
    }
...
mutation.table <- function(seqz.tab, mufreq.threshold = 0.15, min.reads = 40, min.reads.normal = 10,
    max.mut.types = 3, min.type.freq = 0.9, min.fw.freq = 0, segments = NULL) {
    # Input validation
    if (missing(seqz.tab) || nrow(seqz.tab) == 0) {
        stop("Input seqz.tab must be a non-empty data frame")
    }

    # Required columns check
    required_cols <- c("chromosome", "position", "zygosity.normal", "AB.tumor", "good.reads",
        "depth.normal", "Af", "tumor.strand", "AB.normal", "GC.percent", "adjusted.ratio")
    missing_cols <- setdiff(required_cols, names(seqz.tab))
    if (length(missing_cols) > 0) {
        stop(paste("Missing required columns:", paste(missing_cols, collapse = ", ")))
    }

    # Create a dummy dataframe for cases with no mutations
    create_dummy_dataframe <- function(chroms) {
        data.frame(chromosome = chroms, position = 1, GC.percent = NA, good.reads = NA,
            adjusted.ratio = NA, F = 0, mutation = "NA", stringsAsFactors = FALSE)
    }

    # Initial filtering steps Filter for homozygous normal and non-empty
    # AB.tumor
    ...
    mufreq.filt <- seqz.tab$Af <= (1 - mufreq.threshold)
    seqz.tab <- seqz.tab[mufreq.filt, ]

    # Adjust ratios based on segments if provided
    if (!is.null(segments)) {
        # Validate segments dataframe
        if (!all(c("chromosome", "start.pos", "end.pos", "depth.ratio") %in% names(segments))) {
            stop("Segments dataframe is missing required columns")
        }

        for (i in 1:nrow(segments)) {
            pos.filt <- seqz.tab$chromosome == segments$chromosome[i] & seqz.tab$position >=
                segments$start.pos[i] & seqz.tab$position <= segments$end.pos[i]
            seqz.tab$adjusted.ratio[pos.filt] <- segments$depth.ratio[i]
        }
    }

    # If no rows remain after filtering, return dummy dataframe
    if (nrow(seqz.tab) == 0) {
        return(create_dummy_dataframe(unique(seqz.tab$chromosome)))
    }

    # Calculate mutation fractions
    mu.fracts <- tryCatch({
        mut.fractions(AB.tumor = seqz.tab$AB.tumor, Af = seqz.tab$Af, tumor.strand = seqz.tab$tumor.strand)
    }, error = function(e) {
        warning("Error in calculating mutation fractions: ", e$message)
        return(NULL)
    })

    # If mutation fractions calculation failed, return dummy dataframe
    if (is.null(mu.fracts)) {
        return(create_dummy_dataframe(unique(seqz.tab$chromosome)))
    }

    # Apply additional filtering to mutation fractions
    mufreq.filt <- mu.fracts$freq >= mufreq.threshold
    type.filt <- mu.fracts$base.count <= max.mut.types
    prop.filt <- mu.fracts$maj.base.freq >= min.type.freq

    # Forward strand frequency filtering
    if (!is.na(min.fw.freq)) {
        fw.2 <- sort(c(1 - min.fw.freq, min.fw.freq))
    ...
    # Prepare final mutation type column
    mut.type <- paste(seqz.tab$AB.normal, mu.fracts$base, sep = ">")

    # Prepare final dataframe
    result <- seqz.tab[, c("chromosome", "position", "GC.percent", "good.reads",
        "adjusted.ratio")]
    result$F <- mu.fracts$freq
    result$mutation <- mut.type

    # Apply final filtering and combine with dummy dataframe
    final_result <- rbind(result[mufreq.filt, ], create_dummy_dataframe(unique(seqz.tab$chromosome)))
