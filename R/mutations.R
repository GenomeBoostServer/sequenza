mut.fractions <- function(AB.tumor, Af, tumor.strand) {
    # Input validation
    if (length(AB.tumor) != length(Af) || length(AB.tumor) !=
        length(tumor.strand)) {
    ...
    frequencify <- function(x) {
        ...
        split_data <- unlist(strsplit(unlist(x), split = ".",
            fixed = TRUE))
        ...
        base.name <- substr(split_data[1], 1, nchar(split_data[1]) -
            1)
    ...
    result <- data.frame(base.count = as.integer(n.base.mut),
        maj.base.freq = as.numeric(max.freqs[, 1]), base = as.character(max.freqs[,
            2]), freq = as.numeric(max.freqs[, 3]), fw.freq = as.numeric(max.freqs[,
            4]))

    # Validate result dataframe
    if (nrow(result) != length(AB.tumor)) {
        warning(paste("Mismatch in result rows. Expected:", length(AB.tumor),
            "Actual:", nrow(result)))
    }
...
mutation.table <- function(seqz.tab, mufreq.threshold = 0.15,
    ...
    required_cols <- c("chromosome", "position", "zygosity.normal",
        "AB.tumor", "good.reads", "depth.normal", "Af", "tumor.strand",
        "AB.normal", "GC.percent", "adjusted.ratio")
    ...
    create_dummy_dataframe <- function(chroms) {
        data.frame(chromosome = chroms, position = 1, GC.percent = NA,
            good.reads = NA, adjusted.ratio = NA, F = 0, mutation = "NA",
            stringsAsFactors = FALSE)
    }

    # Initial filtering steps Filter for homozygous normal
    # and non-empty AB.tumor
    ...
    if (!is.null(segments)) {
        ...
        if (!all(c("chromosome", "start.pos", "end.pos", "depth.ratio") %in%
            names(segments))) {
    ...
    # If no rows remain after filtering, return dummy
    # dataframe
    if (nrow(seqz.tab) == 0) {
    ...
    mu.fracts <- tryCatch({
        mut.fractions(AB.tumor = seqz.tab$AB.tumor, Af = seqz.tab$Af,
            tumor.strand = seqz.tab$tumor.strand)
    }, error = function(e) {
        warning("Error in calculating mutation fractions: ",
            e$message)
        return(NULL)
    })

    # If mutation fractions calculation failed, return
    # dummy dataframe
    ...
    result <- seqz.tab[, c("chromosome", "position", "GC.percent",
        "good.reads", "adjusted.ratio")]
    result$F <- mu.fracts$freq
    result$mutation <- mut.type

    # Apply final filtering and combine with dummy
    # dataframe
