...
f_y_prima <- function(y, K = 3, U = 10) {
    (K - 3/2) + 1/(2 * exp(y)) + exp(y) * digamma(exp(y)) - exp(y) * digamma(exp(y) +
        U)
}
...
beta.f_y_prima <- function(y, S, W) {
    ...
    r <- r - (3/2) * exp(y)/(exp(y) - F + 1) + (R * F * exp(y))/2 + R * F * exp(y) *
        (y - log(2))/2
    r <- r + (exp(y)/2) * sum_SW
...
sample_beta_a0 <- function(S, W) {
    # Sample the degrees of freedom of the Wishart given a scale matrix W and
    # some observed precision matrices S make it compatible with the
    # one-dimensional case in case we want to use this version for both the
    # multi and the uni-dimensional
    if (is.matrix(W)) {
        xlb <- log(dim(W)[1] - 0.9)
    }
    gen <- ars.new(beta.f_y, beta.f_y_prima, lb = xlb, ub = Inf, S = S, W = W)
    exp(ur(gen, 1))
}
