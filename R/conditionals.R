...
sample_S_ar <- function(A_r, mu_ar_k, beta_a0, W_a0) {
    # Sample attributes covariance matrix of cluster r from its conditional
    # probability
    n <- dim(A_r)[2]
    D <- dim(A_r)[1]
    ...
    # If nobody in the cluster sample from prior
    if (n == 0) {
        # unfortunately wishart implementation does not accept dof > F-1 but
        # dof >= F This will only affect when the cluster is empty.
        df <- max(beta_a0, D)
...
sample_mu_a0 <- function(Lambda_a, mu_a, mu_ar, R_a0) {
    ...
    return(rmvnorm(1, mu_post, Sigma_post)[1, ])
}
...
sample_R_a0 <- function(Sigma_a, mu_ar, mu_a0) {
    ...
    K <- dim(mu_ar)[2]
    D <- dim(mu_ar)[1]
    # Can't use the trick cov(t(mu_ar-mu_a0))*(K-1) when there is only one
    # cluster
    scatter_matrix <- 0
    for (i in 1:K) {
        scatter_matrix <- scatter_matrix + (mu_ar[, i] - mu_a0) %*% t(mu_ar[, i] -
            mu_a0)
    }
