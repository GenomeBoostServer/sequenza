ClusterTraces <- setRefClass(
    "ClusterTraces", fields = c(
        traces = "matrix", traces.z = "matrix", tot_labels = "numeric"
    ),
    methods = c(
        cluster = function(discard = 0.1) {
            .Call(
                `_sequenza_get_clusters`, traces.z, tot_labels,
                discard
            )
        }, cluster_stats = function(discard = 0.1) {
            st <- .Call(
                `_sequenza_get_clusters_stats`, traces.z, tot_labels,
                discard
            )
...
gibbs <- function(
    A, z_init, iters = 100, progressbar = TRUE, plots = FALSE,
    plots.freq = 100
) {
    ...
    for (i in 1:iters) {
        ...
        W_a0 <- sample_W_a0(
            Lambda_a, S_ar[, , active_clusts, drop = FALSE],
            beta_a0
        )
        beta_a0 <- sample_beta_a0(S_ar[, , active_clusts, drop = FALSE], W_a0)

        res <- sample_z_loop(
            N, A, alpha, z, mu_ar, S_ar, mu_a0, R_a0, beta_a0,
            W_a0, init_clust
        )
        z <- res$z
        mu_ar <- res$mu_ar
    ...
    return(
        ClusterTraces$new(
            traces = traces, traces.z = traces.z, tot_labels = init_clust
        )
    )
}

ellipse <- function(
    mu, sigma, alpha = 0.05, npoints = 250, newplot = FALSE,
    draw = TRUE, ...
) {
    es <- eigen(sigma)
            ...
            if (j > i) {
                ...
                for (k in active_clusts) {
                  ellipse(
                    ...
                    sigma = W_a0[c(i, j),
                      c(i, j)],
                    alpha = 0.25, lwd = 5, npoints = 250, col = "black"
                )
                  ellipse(
                    mu = mu_ar[c(j, i),
                      k], sigma = solve(
                    ...
                    alpha = 0.25, lwd = 3, npoints = 250, col = k
                )
                }
            } else if (i == j) {
                plot(
