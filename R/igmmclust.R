ClusterTraces <- setRefClass("ClusterTraces", fields = c(
  traces = "matrix", traces.z = "matrix",
  tot_labels = "numeric"
), methods = c(cluster = function(discard = 0.1) {
...
gibbs <- function(A, z_init, iters = 100, progressbar = TRUE, plots = FALSE, plots.freq = 100) {
  D <- dim(A)[1]
  N <- dim(A)[2]
  ...
  colnames(traces) <- c(
    "n_clusters", "alpha", "mean_mu_a0", "det(R_a0)", "det(W_a0)",
    "beta_a0"
  )
  ...
  if (progressbar) {
    pb <- timerProgressBar(
      min = 1, max = iters, initial = 1, style = 3, char = "+",
      width = 50
    )
  }
  for (i in 1:iters) {
    ...
    for (k in active_clusts) {
      mask <- (z == k)
      S_ar[, , k] <- sample_S_ar(
        A[, mask, drop = FALSE], mu_ar[, k, drop = FALSE],
        beta_a0, W_a0
      )
      mu_ar[, k] <- sample_mu_ar(
        A[, mask, drop = FALSE], S_ar[, , k], mu_a0,
        R_a0
      )
    }
    ...
    # Sample hyperpriors
    mu_a0 <- sample_mu_a0(
      Lambda_a, mu_a, mu_ar[, active_clusts, drop = FALSE],
      R_a0
    )
    R_a0 <- sample_R_a0(Sigma_a, mu_ar[, active_clusts, drop = FALSE], mu_a0)
    W_a0 <- sample_W_a0(Lambda_a, S_ar[, , active_clusts, drop = FALSE], beta_a0)
    beta_a0 <- sample_beta_a0(S_ar[, , active_clusts, drop = FALSE], W_a0)

    res <- sample_z_loop(
      N, A, alpha, z, mu_ar, S_ar, mu_a0, R_a0, beta_a0, W_a0,
      init_clust
    )
    z <- res$z
    mu_ar <- res$mu_ar
    ...
    traces[i, ] <- c(
      length(unique(z)), alpha, mean(mu_a0), det(R_a0), det(W_a0),
      beta_a0
    )
    traces.z[i, ] <- z
    if (plots && (i %% plots.freq == 0)) {
      pairs_ellipses(A, z, i, mu_a0, W_a0, mu_ar, active_clusts, S_ar)
    }
  }
  if (progressbar) {
    closepb(pb)
  ...
  return(ClusterTraces$new(traces = traces, traces.z = traces.z, tot_labels = init_clust))
}

ellipse <- function(
  mu, sigma, alpha = 0.05, npoints = 250, newplot = FALSE, draw = TRUE,
  ...
) {
  es <- eigen(sigma)
...
pairs_ellipses <- function(A, z, iter, mu_a0, W_a0, mu_ar, active_clusts, S_ar) {
  n_size <- nrow(A)
  par(mfrow = c(n_size, n_size), mar = c(0.5, 0.5, 0.5, 0.5), oma = c(
    2, 2, 6,
    2
  ))
  ...
  for (i in 1:n_size) {
    for (j in 1:n_size) {
      if (j > i) {
        ...
        for (k in active_clusts) {
          ellipse(
            mu = mu_a0[c(j, i)], sigma = W_a0[c(i, j), c(i, j)], alpha = 0.25,
            lwd = 5, npoints = 250, col = "black"
          )
          ellipse(mu = mu_ar[c(j, i), k], sigma = solve(S_ar[c(i, j), c(
            i,
            j
          ), k]), alpha = 0.25, lwd = 3, npoints = 250, col = k)
        }
      } else if (i == j) {
        plot(t(A[c(j, i), ]),
