...
gibbs <- function(A, z_init, iters = 100, progressbar = TRUE, plots = FALSE, plots.freq = 100) {
    ...
    colnames(traces) <- c("n_clusters", "alpha", "mean_mu_a0", "det(R_a0)", "det(W_a0)",
        "beta_a0")
    ...
    if (progressbar) {
        pb <- timerProgressBar(min = 1, max = iters, initial = 1, style = 3, char = "+",
            width = 50)
    }
    for (i in 1:iters) {
        ...
        for (k in active_clusts) {
            mask <- (z == k)
            S_ar[, , k] <- sample_S_ar(A[, mask, drop = FALSE], mu_ar[, k, drop = FALSE],
                beta_a0, W_a0)
            mu_ar[, k] <- sample_mu_ar(A[, mask, drop = FALSE], S_ar[, , k], mu_a0,
                R_a0)
        }

        # Sample hyperpriors
        mu_a0 <- sample_mu_a0(Lambda_a, mu_a, mu_ar[, active_clusts, drop = FALSE],
            R_a0)
        ...
        res <- sample_z_loop(N, A, alpha, z, mu_ar, S_ar, mu_a0, R_a0, beta_a0, W_a0,
            init_clust)
        z <- res$z
        mu_ar <- res$mu_ar
        S_ar <- res$S_ar
        ...
        traces[i, ] <- c(length(unique(z)), alpha, mean(mu_a0), det(R_a0), det(W_a0),
            beta_a0)
        traces.z[i, ] <- z
