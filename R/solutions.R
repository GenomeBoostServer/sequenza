get.ci <- function(cp.table, level = 0.95) {
    ...
    values.x <- data.frame(x = cp.table$ploidy, y = apply(cp.table$lpp,
        1, max))
...
alternative.cp.solutions <- function(cp.table) {
    ci <- get.ci(cp.table)
    p.alt <- which(diff(sign(diff(ci$values.ploidy$y))) == -2) +
        1
    get.alt <- function(idx.p, cp.table) {
        idx.c <- which.max(cp.table$lpp[idx.p, ])
        c(cellularity = cp.table$cellularity[idx.c], ploidy = cp.table$ploidy[idx.p],
    ...
    if (nrow(res) > 0) {
        ...
        data.frame(cellularity = ci$max.cellularity, ploidy = ci$max.ploidy,
