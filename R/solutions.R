...
alternative.cp.solutions <- function(cp.table) {
    ...
    if (nrow(res) > 0) {
        ...
        data.frame(cellularity = ci$max.cellularity, ploidy = ci$max.ploidy, SLPP = cp.table$lpp[which(cp.table$ploidy ==
            ci$max.ploidy), which(cp.table$cellularity == ci$max.cellularity)])
    }
}
