theoretical.depth.ratio <- function(
    CNt, cellularity, ploidy, CNn = 2, normal.ploidy = 2, avg.depth.ratio = 1
) {
...
mufreq.types.matrix <- function(CNt.min, CNt.max, CNn = 2) {
    ...
    data.frame(
        CNn = CNn, CNt = rep(CNt, times = times_muts),
        Mt = unlist(mut_comb)
    )
}
...
baf.model.points <- function(cellularity, ploidy, baf_types, avg.depth.ratio) {
    depth_ratio <- theoretical.depth.ratio(
        cellularity = cellularity, ploidy = ploidy, CNn = baf_types[,
            "CNn"], CNt = baf_types[, "CNt"], avg.depth.ratio = avg.depth.ratio
    )
    baf <- theoretical.baf(
        cellularity = cellularity, CNn = baf_types[, "CNn"],
...
mufreq.model.points <- function(cellularity, ploidy, mufreq_types, avg.depth.ratio) {
    mufreqs <- theoretical.mufreq(
        cellularity = cellularity, CNn = mufreq_types[, "CNn"],
        CNt = mufreq_types[, "CNt"], Mt = mufreq_types[, "Mt"]
    )
    depth_ratio <- theoretical.depth.ratio(
        cellularity = cellularity, ploidy = ploidy, CNn = mufreq_types[,
            "CNn"], CNt = mufreq_types[, "CNt"], avg.depth.ratio = avg.depth.ratio
    )
...
b_allele_freq <- function(Af, Bf, good.reads, conf = 0.95) {
    if (length(Bf) >
        ...
        dd <- density(
            c(Bf, Af),
            weight = c(good.reads, good.reads)/(2 *
                sum(good.reads))
        )
        points.max <- which(
            diff(sign(diff(dd$y))) ==
