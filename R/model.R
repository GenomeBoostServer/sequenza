...
theoretical.depth.ratio <- function(CNt, cellularity, ploidy, CNn = 2, normal.ploidy = 2,
    avg.depth.ratio = 1) {
    ...
    ploidy_cellu_term <- (ploidy/normal.ploidy * cellularity) + (1 - cellularity)
...
theoretical.baf <- function(CNt, B, cellularity, CNn = 2) {
    ...
    baf <- ((B * cellularity) + (1 - cellularity))/((CNt * cellularity) + CNn * (1 -
        cellularity))
...
mufreq.types.matrix <- function(CNt.min, CNt.max, CNn = 2) {
    ...
    data.frame(CNn = CNn, CNt = rep(CNt, times = times_muts), Mt = unlist(mut_comb))
}
...
baf.model.points <- function(cellularity, ploidy, baf_types, avg.depth.ratio) {
    ...
    depth_ratio <- theoretical.depth.ratio(cellularity = cellularity, ploidy = ploidy,
        CNn = baf_types[, "CNn"], CNt = baf_types[, "CNt"], avg.depth.ratio = avg.depth.ratio)

    # Calculate theoretical BAF values
    baf <- theoretical.baf(cellularity = cellularity, CNn = baf_types[, "CNn"], CNt = baf_types[,
        "CNt"], B = baf_types[, "B"])
...
mufreq.model.points <- function(cellularity, ploidy, mufreq_types, avg.depth.ratio) {
    # Calculate theoretical mutation frequencies
    mufreqs <- theoretical.mufreq(cellularity = cellularity, CNn = mufreq_types[,
        "CNn"], CNt = mufreq_types[, "CNt"], Mt = mufreq_types[, "Mt"])

    # Calculate theoretical depth ratios
    depth_ratio <- theoretical.depth.ratio(cellularity = cellularity, ploidy = ploidy,
        CNn = mufreq_types[, "CNn"], CNt = mufreq_types[, "CNt"], avg.depth.ratio = avg.depth.ratio)
...
b_allele_freq <- function(Af, Bf, good.reads, conf = 0.95) {
    ...
    if (length(Bf) > 1) {
        ...
        # If no strong maxima found or global maximum not among them, look for
        # weaker local maxima
        ...
        # Find the B-allele value at the highest density peak
        ...
        # Adjust B-allele frequency if it's greater than 0.5
        if (b.val > 0.5) {
            b.val <- 1 - b.val
        }
        ...
        # Return lower bound, B-allele frequency, and upper bound Lower CI
        return(c(b.val - interval_width, b.val, b.val + interval_width))

    } else if (length(Bf) == 1) {
        # If only one B-allele frequency, return it with no uncertainty
        return(c(Bf, Bf, Bf))
