...
gc_data_smooth <- function(gc_list, min_times = 20, n = 100, scale.subset = 1.5,
    ...) {
    ...
    regrid <- smooth_matrix(x = gc_list$gc, y = gc_list$depth[gc_list$depth <= max_depth],
    ...
    list(gc = regrid$eval.points[[1]], depth = regrid$eval.points[[2]], n = n_tab)
}

get_gc <- function(gc_col, smooth = TRUE, min_times = 20, grid_size = 100, scale.subset = 1.5,
    ...) {
    ...
    n[is.na(n)] <- 0
    if (smooth == TRUE) {
        gc_data_smooth(list(gc = as.numeric(names_gc), depth = as.numeric(names_depths),
            n = n), min_times = min_times, n = grid_size, scale.subset = scale.subset,
            ...)
    } else {
...
gc_table <- function(x, y, args) {
    lapply(split(y[, x[1]], y[, x[2]]), table)
}
...
do_get_gc <- function(x, args) {
    ...
    get_gc(x, smooth = smooth, min_times = min_times, grid_size = grid_size, scale.subset = scale_subset)
}

gc.sample.stats <- function(file, col_types = "c--dd----d----", buffer = 33554432,
    parallel = 2L, stats = TRUE, smooth = TRUE, min_times = 20, n = 100, scale.subset = 1.5,
    verbose = TRUE, ...) {

    get_gc_args <- list(smooth = smooth, min_times = min_times, grid_size = n, scale_subset = scale.subset)

    data_fast_stats(file, col_types = col_types, buffer = buffer, parallel = parallel,
        stats = stats, verbose = verbose, col_sets = list(normal = c(2, 4), tumor = c(3,
            4)), f1 = gc_table, f2 = do_get_gc, args_f1 = list(), args_f2 = get_gc_args,
        msg = "Collecting GC information ")
}

unfold_gc <- function(x, stats = FALSE, smooth = TRUE, min_times = 20, cl = 2L, grid_size = 100) {
    get_gc_args <- list(smooth = smooth, min_times = min_times, grid_size = grid_size,
        scale_subset = 1.5)
    unfold_data(x = x, f = do_get_gc, args_f = get_gc_args, stats = stats)
}

unfold_gc_old <- function(x, stats = TRUE, smooth = TRUE, min_times = 20, grid_size = 100,
    scale.subset = 1.5, ...) {
    gc_norm <- get_gc(x[, "gc_nor"], smooth, min_times, grid_size, scale.subset,
        ...)
    gc_tum <- get_gc(x[, "gc_tum"], smooth, min_times, grid_size, scale.subset, ...)
    if (stats) {
        ...
        stats_chrom <- data.frame(chr = ord_chrom, n_lines = stats_chrom, start = stats_start,
            end = stats_end)
