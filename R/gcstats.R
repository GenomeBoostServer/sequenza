...
gc_data_smooth <- function(
  gc_list, min_times = 20, n = 100, scale.subset = 1.5,
  ...
) {
  ...
  regrid <- smooth_matrix(
    x = gc_list$gc, y = gc_list$depth[gc_list$depth <= max_depth],
...
get_gc <- function(
  gc_col, smooth = TRUE, min_times = 20, grid_size = 100, scale.subset = 1.5,
  ...
) {
  ...
  if (smooth == TRUE) {
    gc_data_smooth(
      list(
        gc = as.numeric(names_gc), depth = as.numeric(names_depths),
        n = n
      ),
...
gc.sample.stats <- function(
  ...
  data_fast_stats(file,
    col_types = col_types, buffer = buffer, parallel = parallel,
    stats = stats, verbose = verbose, col_sets = list(normal = c(2, 4), tumor = c(
      3,
      4
    )), f1 = gc_table, f2 = do_get_gc, args_f1 = list(), args_f2 = get_gc_args,
...
unfold_gc_old <- function(
  x, stats = TRUE, smooth = TRUE, min_times = 20, grid_size = 100,
  scale.subset = 1.5, ...
) {
  gc_norm <- get_gc(
    x[, "gc_nor"], smooth, min_times, grid_size, scale.subset,
    ...
  )
