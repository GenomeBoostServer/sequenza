...
gc_data_smooth <- function(gc_list, min_times = 20, n = 100, scale.subset = 1.5, ...) {
    ...
    list(
        gc = regrid$eval.points[[1]], depth = regrid$eval.points[[2]],
        n = n_tab
    )
}
...
get_gc <- function(
    ...
    n <- do.call(
        rbind, lapply(
            all_depths, FUN = function(x, names_depths) {
                res <- x[names_depths]
    ...
    if (smooth == TRUE) {
        gc_data_smooth(
            list(
                gc = as.numeric(names_gc),
                depth = as.numeric(names_depths),
                n = n
            ),
...
gc.sample.stats <- function(
    ...
    stats = TRUE, smooth = TRUE, min_times = 20, n = 100, scale.subset = 1.5,
    verbose = TRUE, ...
) {

    get_gc_args <- list(
        smooth = smooth, min_times = min_times, grid_size = n,
        scale_subset = scale.subset
    )

    data_fast_stats(
        file, col_types = col_types, buffer = buffer, parallel = parallel,
        stats = stats, verbose = verbose, col_sets = list(
            normal = c(2, 4),
            tumor = c(3, 4)
        ),
...
unfold_gc <- function(
    x, stats = FALSE, smooth = TRUE, min_times = 20, cl = 2L,
    grid_size = 100
) {
    get_gc_args <- list(
...
unfold_gc_old <- function(
    ...
    gc_tum <- get_gc(
        x[, "gc_tum"], smooth, min_times, grid_size, scale.subset,
        ...
    )
    if (stats) {
        ...
        list(
            file.metrics = stats_chrom, normal = gc_norm, tumor = gc_tum
        )
    } else {
