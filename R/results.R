sequenza.results <- function(
    ...
    assign(
        x = paste0(sample.id, "_sequenza_extract"),
        value = sequenza.extract
    )
    save(
        list = paste0(sample.id, "_sequenza_extract"),
        file = robj.extr
    )
    if (is.null(cp.table) &&
        (is.null(cellularity) ||
            is.null(ploidy))) {
        stop(
            "cp.table and/or cellularity and ploidy argument are required."
        )
    }
    ...
    par(mfrow = c(1, 2))
    gc.summary.plot(
        sequenza.extract$gc$normal, mean.col = "lightsalmon",
        median.col = "lightgreen", las = 1, xlab = "GC %", ylab = "Depth",
        zlab = "N", main = "GC vs raw depth in the normal sample"
    )
    gc.summary.plot(
        sequenza.extract$gc_norm$normal, mean.col = "lightsalmon",
        median.col = "lightgreen", las = 1, xlab = "GC %", ylab = "Depth",
        zlab = "N", main = "GC vs normalized depth in the normal sample"
    )
    gc.summary.plot(
        sequenza.extract$gc$tumor, mean.col = "lightsalmon",
        median.col = "lightgreen", las = 1, xlab = "GC %", ylab = "Depth",
        zlab = "N", main = "GC vs raw depth in the tumor sample"
    )
    gc.summary.plot(
        sequenza.extract$gc_norm$tumor, mean.col = "lightsalmon",
        median.col = "lightgreen", las = 1, xlab = "GC %", ylab = "Depth",
    ...
    for (i in unique(seg.tab$chromosome)) {
        ...
        par(
            mfcol = c(3, 2),
            xaxt = "n", mar = c(0, 4, 3, 0),
            oma = c(5, 0, 4, 0)
        )
        plotWindows(
            sequenza.extract$depths$raw$normal[[i]], ylab = "normal depth",
            ylim = c(0, 2.5),
            main = paste("raw", i, sep = " ")
        )
        plotWindows(
            sequenza.extract$depths$raw$tumor[[i]], ylab = "tumor depth",
            ylim = c(0, 2.5)
        )
        plotWindows(
            sequenza.extract$raw_ratio[[i]], ylab = "depth ratio",
            ylim = c(0, 2.5)
        )
        par(xaxt = "s")
        axis(
            ...
            side = 1, line = 0, at = seq(0, max_coord_chr_i, by = 1e+07),
            outer = FALSE, cex = par("cex.axis") *
                par("cex")
        )
        mtext(
            "Position (Mb)", side = 1, line = 3, outer = FALSE,
            cex = par("cex.lab") *
                par("cex")
        )
        par(xaxt = "n")
        plotWindows(
            sequenza.extract$depths$norm$normal[[i]], ylab = "normal depth",
            ylim = c(0, 2.5),
            main = paste("normalized", i, sep = " ")
        )
        plotWindows(
            sequenza.extract$depths$norm$tumor[[i]], ylab = "tumor depth",
            ylim = c(0, 2.5)
        )
        plotWindows(
            sequenza.extract$ratio[[i]], ylab = "depth ratio",
            ylim = c(0, 2.5)
        )
        par(xaxt = "s")
        axis(
    ...
    if (!is.null(cp.table)) {
        assign(
            x = paste0(sample.id, "_sequenza_cp_table"),
            value = cp.table
        )
        save(
            list = paste0(sample.id, "_sequenza_cp_table"),
            file = robj.fit
        )
        ...
        cp.plot(cp.table)
        cp.plot.contours(
            cp.table, add = TRUE, likThresh = c(0.95),
            col = "red", pch = 20
        )
        if (!is.null(cellularity) ||
            ...
            text(
                x = ploidy, y = cellularity, labels = "User selection",
                pos = 3, offset = 0.5
            )
        } else {
            cellularity <- cint$max.cellularity
    ...
    write.table(
        seg.res, file = segs.file, col.names = TRUE, row.names = FALSE,
        sep = "\t", quote = FALSE
    )
    if (nrow(mut.tab) >
        0) {
        ...
        write.table(
            mut.res, file = muts.file, col.names = TRUE, row.names = FALSE,
            sep = "\t", quote = FALSE
        )
    }
    pdf(chrw.file)
    for (i in unique(seg.res$chromosome)) {
        ...
        chromosome.view(
            ...
            ploidy = ploidy, main = i, segments = seg.res[seg.res$chromosome ==
                i, ], avg.depth.ratio = avg.depth.ratio, CNn = CNn,
            min.N.ratio = 1
        )
    }
    dev.off()
    ...
    if (!is.null(sequenza.extract$ratio_baf_raster)) {
        ...
        image(
            sequenza.extract$ratio_baf_raster$smooth, xlim = c(0, 0.505),
            ylim = c(0, 3),
            xlab = "BAF", ylab = "normalised depth-ratio", main = "Smoothed densitt"
        )
        peaks_dens <- find_local_max(
            sequenza.extract$ratio_baf_raster$smooth, f_threshold = 0.05
        )
        points(
            peaks_dens[, 1], peaks_dens[, 2], cex = peaks_dens[,
                3], pch = 21, bg = "grey"
        )
        image(
            sequenza.extract$ratio_baf_raster$raster, xlim = c(0, 0.505),
            ylim = c(0, 3),
            xlab = "BAF", ylab = "normalised depth-ratio", main = "Binned data"
        )
        peaks_dens <- find_local_max(
            sequenza.extract$ratio_baf_raster$raster, f_threshold = 0.05
        )
        points(
            peaks_dens[, 1], peaks_dens[, 2], cex = peaks_dens[,
                3], pch = 21, bg = "grey"
    ...
    if (sum(!is.na(seg.res$A)) >
        0) {
        genome.view(seg.res)
    }
    genome.view(seg.res, "CN")
    plotRawGenome(
        sequenza.extract, cellularity = cellularity, ploidy = ploidy,
        mirror.BAF = TRUE
    )
    dev.off()
    barscn <- data.frame(
        size = seg.res$end.pos - seg.res$start.pos, CNt = seg.res$CNt
    )
    ...
    if (!is.null(cp.table)) {
        res.tab <- data.frame(
            ...
            ploidy.estimate = c(
                cint$confint.ploidy[1], cint$max.ploidy[1], cint$confint.ploidy[2]
            ),
        ...
        write.table(
            res.tab, cint.file, col.names = TRUE, row.names = FALSE,
            sep = "\t", quote = FALSE
        )
    }
    pdf(fit.file, width = 6, height = 6)
    baf.model.view(
        cellularity = cellularity, ploidy = ploidy, segs = seg.res[!segs.is.xy,
            ]
    )
    dev.off()
    if (!is.null(cp.table)) {
        ...
        write.table(
            alt.sol, file = alt.file, col.names = TRUE, row.names = FALSE,
            sep = "\t", quote = FALSE
        )
        pdf(afit.file)
    ...
    writeLines(
        c(
            date(), paste(
                "Sequenza version:", packageVersion("sequenza"),
                sep = " "
            )
        ),
        file_conn
