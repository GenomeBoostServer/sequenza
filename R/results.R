...
sequenza.results <- function(sequenza.extract, cp.table = NULL, sample.id, out.dir = getwd(),
  ...
  # Process segments
  segments_data <- process_segments_data(sequenza.extract, chromosome.list)
  ...
  cp_results <- process_cp_table(cp.table, cellularity, ploidy, files, sample.id)
...
setup_output_dir <- function(out_dir) {
  if (!file.exists(out_dir)) {
    ...
    if (!dir_ok) {
      stop("Directory does not exist and cannot be created: ", out_dir)
    }
  }
}
...
save_extraction_data <- function(sequenza.extract, robj.extr, sample.id) {
...
plot_depths <- function(sequenza.extract, seg.tab, depths.file) {
  ...
  for (i in unique(seg.tab$chromosome)) {
    ...
    plotWindows(sequenza.extract$raw_ratio[[i]], ylab = "depth ratio", ylim = c(
      0,
      2.5
    ))
    par(xaxt = "s")
    axis(
      labels = as.character(round(
        seq(0, max_coord_chr_i / 1e+06, by = 10),
        0
      )), side = 1, line = 0, at = seq(0, max_coord_chr_i, by = 1e+07), outer = FALSE,
    ...
    plotWindows(sequenza.extract$ratio[[i]], ylab = "depth ratio", ylim = c(
      0,
      2.5
    ))
    par(xaxt = "s")
...
process_cp_table <- function(cp.table, cellularity, ploidy, files, sample.id) {
  if (!is.null(cp.table)) {
    assign(x = paste0(sample.id, "_sequenza_cp_table"), value = cp.table)
    save(list = paste0(sample.id, "_sequenza_cp_table"), file = files$robj.fit)
    cint <- get.ci(cp.table)
    pdf(files$cp.file)
    cp.plot(cp.table)
    cp.plot.contours(cp.table,
      add = TRUE, likThresh = c(0.95), col = "red",
      pch = 20
    )
    if (!is.null(cellularity) || !is.null(ploidy)) {
      ...
      text(
        x = ploidy, y = cellularity, labels = "User selection", pos = 3,
        offset = 0.5
      )
    } else {
      cellularity <- cint$max.cellularity
...
process_mutations_and_segments <- function(sequenza.extract, segments_data, cp_results, female, XY, CNt.max, ratio.priority, chromosome.list, files) {
  ...
  cn.alleles <- baf.bayes(
    Bf = seg.tab$Bf[!segs.is.xy], CNt.max = CNt.max, depth.ratio = seg.tab$depth.ratio[!segs.is.xy],
  ...
  if (!female) {
    if (sum(segs.is.xy) >= 1) {
      cn.alleles <- baf.bayes(
        Bf = NA, CNt.max = CNt.max, depth.ratio = seg.tab$depth.ratio[segs.is.xy],
        cellularity = cellularity, ploidy = ploidy, avg.depth.ratio = avg.depth.ratio,
        sd.ratio = seg.tab$sd.ratio[segs.is.xy], weight.ratio = seg.len[segs.is.xy],
  ...
  if (nrow(mut.tab) > 0) {
    mut.alleles <- mufreq.bayes(
      mufreq = mut.tab$F[!mut.is.xy], CNt.max = CNt.max,
    ...
    write.table(mut.res,
      file = files$muts.file, col.names = TRUE, row.names = FALSE,
      sep = "\t", quote = FALSE
    )
  }
...
generate_result_plots <- function(results, files, sequenza.extract, cellularity, ploidy, female, cp_results, XY) {
  ...
  # Define segs.is.xy here for use throughout the function
  if (female) {
...
write_log_file <- function(log.file) {
  ...
  writeLines(
    c(date(), paste("Sequenza version:", packageVersion("sequenza"), sep = " ")),
    file_conn
  )
  close(file_conn)
}
...
validate_results_input <- function(sequenza.extract, sample.id, out.dir) {
