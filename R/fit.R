sequenza.fit <- function(
    ...
    segment.filter = 3e+06, mufreq.treshold = 0.1, XY = c(X = "X", Y = "Y"),
    ...
    if (method == "baf") {
        ...
        baf.model.fit(
            Bf = seg.test$Bf, depth.ratio = seg.test$depth.ratio,
        ...
        mufreq.model.fit(
            mufreq = mut.test$F, depth.ratio = mut.test$adjusted.ratio,
