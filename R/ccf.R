...
sim_subclonal_segs <- function(
  CNn, CNt, CNst, N, cellularity, ploidy, CCF, depth,
  ...
) {
...
depth_ratio_dpois_ccf <- function(
  x, CNn, CNt, CNst, cellularity, ploidy, CCF, depth,
...
depth_ratio_ccf_likelihood <- function(
  x, CCFs, CNn, CNt, CNst, cellularity, ploidy,
  depth, avg.depth.ratio = 1, normal.ploidy = 2
) {
  sapply(CCFs,
    FUN = function(x, r, CNn, CNt, CNst, cellularity, ploidy, depth,
                   avg.depth.ratio, normal.ploidy) {
      depth_ratio_dpois_ccf(
        r, CNn, CNt, CNst, cellularity, ploidy, x, depth, avg.depth.ratio,
        normal.ploidy
      )
...
#'
#' @export
ccf_per_segment <- function(
  segs_tab, CCFs, cellularity, ploidy, avg_depth, CNn,
  avg.depth.ratio, CNst_diff = 1, normal.ploidy = 2, ...
) {
  # Generate BAF/ratio model points
  types_matrix <- baf.types.matrix(0, max(segs_tab$CNt) + CNst_diff, CNn)
  types_values <- baf.model.points(cellularity, ploidy, types_matrix, avg.depth.ratio)
  ...
  i_sCNt <- ifelse(type_ratios >= depth_ratios, CNt_values - CNst_diff, CNt_values +
    CNst_diff)

  # Calculate CCF likelihoods for all segments
  ccf_results <- t(mapply(function(ratio, cnt, scnt, depth) {
    likelihood <- depth_ratio_ccf_likelihood(
      x = ratio, CCFs = CCFs, CNn = CNn,
      CNt = cnt, CNst = scnt, cellularity = cellularity, ploidy = ploidy, depth = depth,
