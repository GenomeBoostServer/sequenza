...
get_assembly <- function(name, url = NULL, prefix = "chr") {
  ...
  cytobands <- read_tsv(file_path, col_types = "ciicc", col_names = c(
    "chromosome",
...
slide_matrix <- function(x, position = NULL, w = 100, smooth = TRUE, method = c(
                           "kstest",
                           "meandiff", "both"
                         ), verbose = TRUE) {
  if (is.null(position)) {
  ...
  apply(arms, 1, FUN = function(arm) {
    ...
    if (sum(index) > 0) {
      ...
      if (length(coords) > 0) {
        ...
        (breaks <- tibble(start.pos = min(position[index]), end.pos = max(position[index])))
      }
      breaks
    }
  })
