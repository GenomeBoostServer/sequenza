...
select_chromosomes_with_centromere <- function(assembly, all_sequences) {
    ...
    if (sum(select_sequences) ==
        0) {
        select_sequences <- paste0("chr", all_sequences) %in%
            chromosomes
    }
...
get_arms <- function(chromosome) {
    ...
    if (sum(acen) ==
        ...
        res <- add_row(
            res, chromosome = chrom, start = q_start, end = end,
            arm = "q"
        )
    }
    res
...
get_assembly <- function(name, url = NULL, prefix = "chr") {
    ...
    cytobands <- read_tsv(
        file_path, col_types = "ciicc", col_names = c("chromosome", "start", "end", "cytoband", "type")
    )
    arms <- do.call(
        rbind, lapply(
            split(cytobands, cytobands$chromosome),
            get_arms
        )
    )
...
slide_matrix <- function(
    x, position = NULL, w = 100, smooth = TRUE, method = c("kstest", "meandiff", "both"),
    verbose = TRUE
) {
    ...
    .Call(
        `_sequenza_slide_matrix`, x, position, w, smooth, method,
        verbose
    )
}
...
get_gaps_peaks <- function(x, w = 100, position = NULL, arms) {
    apply(
        arms, 1, FUN = function(arm) {
            index <- between(
                position, as.numeric(arm["start"]),
                as.numeric(arm["end"])
            )
            if (sum(index) >
                0) {
                ...
                if (length(coords) >
                  ...
                  breaks <- add_row(
                    breaks, start.pos = min(position[index]),
                    end.pos = coords[1] - 1, .before = 1
                )
                  breaks <- add_row(
                    breaks, start.pos = coords[length(coords)] +
                      1, end.pos = max(position[index])
                )
                } else (breaks <- tibble(
                  start.pos = min(position[index]),
