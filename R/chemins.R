...
get_arms <- function(chromosome) {
    ...
    if (sum(acen) == 0) {
        res <- tibble(chromosome = chrom, start = start, end = end,
            arm = "n")
    } else {
        p_end <- min(chromosome$start[acen])
        q_start <- max(chromosome$end[acen])
        res <- tibble(chromosome = chrom, start = start, end = p_end,
            arm = "p")
        res <- add_row(res, chromosome = chrom, start = q_start,
            end = end, arm = "q")
    }
    res
}
...
get_assembly <- function(name, url = NULL, prefix = "chr") {
    ...
    arms <- do.call(rbind, lapply(split(cytobands, cytobands$chromosome),
        get_arms))
    arms$chromosome <- str_replace(arms$chromosome, "^chr", prefix)
    return(arms)
}

slide_matrix <- function(x, position = NULL, w = 100, smooth = TRUE,
...
get_gaps_peaks <- function(x, w = 100, position = NULL, arms) {
    apply(arms, 1, FUN = function(arm) {
        index <- between(position, as.numeric(arm["start"]),
            as.numeric(arm["end"]))
        if (sum(index) > 0) {
            coords <- get_peaks(x = x[index], position = position[index],
                w = w)
            if (length(coords) > 0) {
                ...
                breaks <- add_row(breaks, start.pos = coords[length(coords)] +
