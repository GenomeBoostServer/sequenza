find_breaks <- function(seqz.baf, slide_win, peak_win, arms, chr_name, verbose) {
    ...
    ratio_diffs <- slide_matrix(
        seqz.baf$adjusted.ratio, w = slide_win, position = seqz.baf$position,
        verbose = verbose
    )
    if (verbose) {
    ...
    bf_diffs <- slide_matrix(
        seqz.baf$Bf, w = slide_win, position = seqz.baf$position,
        verbose = verbose
    )
...
extract_breaks <- function(
    data, data_het, breaks, slide_win, peak_win, assembly, chromosome,
    verbose = TRUE
) {
    if (is.null(breaks)) {
        golden_path <- paste(
            "http://hgdownload.cse.ucsc.edu", "goldenPath", assembly,
        ...
        find_breaks(
            data_het, slide_win, peak_win, arms_i, chromosome,
            verbose
        )
    } else {
        breaks
...
segment.breaks <- function(seqz.tab, breaks, min.reads.baf = 1, weighted.mean = TRUE) {
    ...
    for (i in 1:length(seqz.tab)) {
        ...
        nb <- nrow(breaks.i)
        breaks.vect <- do.call(
            cbind, split.data.frame(
                breaks.i[, c("start.pos", "end.pos")],
                f = 1:nb
            )
        )
        ...
        breaks.vect <- unique.breaks(
            b = as.numeric(breaks.vect),
            offset = 1
        )
        fact.r.i <- cut(seqz.tab[[i]]$position, breaks.vect)
        fact.b.i <- cut(seqz.b.i$position, breaks.vect)
        seg.i.s.r <- sapply(
            X = split(seqz.tab[[i]]$chromosome, f = fact.r.i),
            FUN = length
        )
        seg.i.s.b <- sapply(
            X = split(seqz.b.i$chromosome, f = fact.b.i),
            FUN = length
        )

        if (weighted.mean) {
            seg.i.rw <- sapply(
                X = split(seqz.tab[[i]]$rw, f = fact.r.i),
                FUN = function(a) sum(a, na.rm = TRUE)
            )
            seg.i.w.r <- sapply(
                X = split(seqz.tab[[i]]$w.r, f = fact.r.i),
                FUN = function(a) sum(a, na.rm = TRUE)
            )
            seg.i.r.sd <- sapply(
                X = split(seqz.tab[[i]]$rw/seqz.tab[[i]]$w.r, f = fact.r.i),
                FUN = function(a) sd(a, na.rm = TRUE)
            )
            seg.i.b.sd <- sapply(
                X = split(seqz.b.i$bw/seqz.b.i$w.b, f = fact.b.i),
                FUN = function(a) sd(a, na.rm = TRUE)
            )
            A.split <- split(seqz.b.i$Af, f = fact.b.i)
            B.split <- split(seqz.b.i$Bf, f = fact.b.i)
            d.split <- split(seqz.b.i$good.reads, f = fact.b.i)
            window.quantiles <- mapply(
                b_allele_freq, Af = A.split, Bf = B.split, good.reads = d.split,
                conf = 0.95
            )
            segments.i <- data.frame(
                chromosome = names(seqz.tab)[i],
            ...
            seg.i.r <- sapply(
                X = split(seqz.tab[[i]]$adjusted.ratio, f = fact.r.i),
            ...
            seg.i.b.sd <- sapply(
                X = split(seqz.b.i$Bf, f = fact.b.i),
                FUN = function(a) sd(a, na.rm = TRUE)
            )
            segments.i <- data.frame(
                chromosome = names(seqz.tab)[i],
                ...
                Bf = window.quantiles[2, ], N.BAF = seg.i.s.b,
                sd.BAF = seg.i.b.sd, depth.ratio = seg.i.r, N.ratio = seg.i.s.r,
                sd.ratio = seg.i.r.sd, stringsAsFactors = FALSE
            )
        }
        segments[[i]] <- segments.i[seq(
            from = 1, to = nrow(segments.i),
            by = 2
        ),
            ]
    }
