find_breaks <- function(seqz.baf, slide_win, peak_win, arms,
    ...
    peaks_both <- get_gaps_peaks(x = (ratio_diffs$y + bf_diffs$y)/2,
        position = ratio_diffs$x, w = peak_win, arms = arms)
    breaks <- lapply(peaks_both, FUN = function(peaks) {
        ...
        if (is.null(coords)) {
            ...
            pos_end <- coords[-1]
            data.frame(chrom = chr_name, start.pos = pos_start,
                end.pos = pos_end)
        }
    })
...
slide_tracks <- function(seqz.baf, slide_win, signal_out = c("both",
    "ratio", "baf"), verbose = TRUE) {
    ...
    if (signal_out %in% c("both", "ratio")) {
        ...
        ratio_diffs <- slide_matrix(seqz.baf$adjusted.ratio,
            w = slide_win, position = seqz.baf$position, verbose = verbose)
    }
...
peaks_tracks <- function(diff_track, peak_win, arms, chr_name,
    verbose) {
    peaks <- get_gaps_peaks(x = diff_track$y, position = diff_track$x,
    ...
    breaks <- do.call(rbind, breaks)
    not_uniq <- which(breaks$end.pos == c(breaks$start.pos[-1],
        0))
...
extract_breaks <- function(data, data_het, breaks, slide_win,
    peak_win, assembly, chromosome, verbose = TRUE) {
    if (is.null(breaks)) {
        golden_path <- paste("http://hgdownload.cse.ucsc.edu",
            "goldenPath", assembly, "database", "cytoBandIdeo.txt.gz",
            sep = "/")
...
extract_breaks_tracks <- function(track, breaks, slide_win, peak_win,
    ...
    if (is.null(breaks)) {
        ...
        arms_i <- arms[arms$chromosome == chr_arm, ]
        # If no arms found for this chromosome, create a
        # fallback
        if (nrow(arms_i) == 0) {
            arms_i <- data.frame(chromosome = chromosome, start = min(track$x,
                na.rm = TRUE), end = max(track$x, na.rm = TRUE),
                arm = "q", stringsAsFactors = FALSE)
        }
...
segment.breaks <- function(seqz.tab, breaks, min.reads.baf = 1,
    ...
    required_cols <- c("chromosome", "position", "zygosity.normal",
        "good.reads", "Af", "Bf", "depth.normal", "adjusted.ratio")
    ...
    if (weighted.mean) {
        # Calculate weighted ratios and B-allele
        # frequencies
        ...
        seqz.tab <- cbind(seqz.tab[, c("chromosome", "position",
            "zygosity.normal", "good.reads", "Af", "Bf")], rw = rw,
            w.r = w.r, bw = bw, w.b = w.b)
    }
    ...
    for (i in seq_along(seqz.tab)) {
        ...
        # Filter heterozygous positions with sufficient
        # reads
        ...
        seg.i.s.r <- sapply(split(seqz.tab[[i]]$chromosome, f = fact.r.i),
            length)
        seg.i.s.b <- sapply(split(seqz.b.i$chromosome, f = fact.b.i),
            length)

        # Calculate segment statistics based on weighting
        # method
        if (weighted.mean) {
            segments.i <- calculate_weighted_segments(seqz_data = seqz.tab[[i]],
                baf_data = seqz.b.i, fact_r = fact.r.i, fact_b = fact.b.i,
                breaks_vect = breaks.vect, chrom_name = current_chrom,
                seg.i.s.r = seg.i.s.r, seg.i.s.b = seg.i.s.b)
        } else {
        ...
        segments[[i]] <- segments.i[seq(1, nrow(segments.i),
            by = 2), ]
    }
...
calculate_weighted_segments <- function(seqz_data, baf_data,
    fact_r, fact_b, breaks_vect, chrom_name, seg.i.s.r, seg.i.s.b) {
    # Calculate weighted sums and standard deviations
    seg.i.rw <- sapply(split(seqz_data$rw, f = fact_r), sum,
        na.rm = TRUE)
    seg.i.w.r <- sapply(split(seqz_data$w.r, f = fact_r), sum,
        na.rm = TRUE)

    # Calculate standard deviations
    seg.i.r.sd <- sapply(split(seqz_data$rw/seqz_data$w.r, f = fact_r),
        sd, na.rm = TRUE)
    seg.i.b.sd <- sapply(split(baf_data$bw/baf_data$w.b, f = fact_b),
        sd, na.rm = TRUE)
    ...
    window.quantiles <- mapply(b_allele_freq, Af = A.split, Bf = B.split,
        good.reads = d.split, conf = 0.95)

    # Create segments dataframe
    data.frame(chromosome = chrom_name, start.pos = as.numeric(breaks_vect[-length(breaks_vect)]),
        end.pos = as.numeric(breaks_vect[-1]), Bf = window.quantiles[2,
...
calculate_unweighted_segments <- function(seqz_data, baf_data,
    fact_r, fact_b, breaks_vect, chrom_name, seg.i.s.r, seg.i.s.b) {
    # Calculate means and standard deviations
    seg.i.r <- sapply(split(seqz_data$adjusted.ratio, f = fact_r),
        mean, na.rm = TRUE)
    seg.i.r.sd <- sapply(split(seqz_data$adjusted.ratio, f = fact_r),
        sd, na.rm = TRUE)
    seg.i.b.sd <- sapply(split(baf_data$Bf, f = fact_b), sd,
        na.rm = TRUE)
...
cluster_segments <- function(bf, depth_ratio, init_clust = 10,
    ...
    sequenza:::gibbs(t(x), z_init = sample(1:init_clust, nrow(x),
        replace = T), ...)
}


process_segments_by_clusters <- function(sequenza_extract, seqz_file,
    out_path, file_out_prefix, init_n_clust = 10, dp_iter = 1000,
    pdf_out = FALSE, verbose = FALSE, ...) {
    segs_i <- do.call(rbind, sequenza_extract$segments)
    gc_stats <- sequenza_extract$gc
    segs_fitting_ratio <- sapply(sequenza_extract$chromosomes,
        FUN = function(x, extr) {
            compare_bins(extr$segments[[x]]$start.pos, extr$segments[[x]]$end.pos,
                extr$segments[[x]]$depth.ratio, extr$ratio[[x]])
        }, extr = sequenza_extract)
    segs_fitting_baf <- sapply(sequenza_extract$chromosomes,
        FUN = function(x, extr) {
            compare_bins(extr$segments[[x]]$start.pos, extr$segments[[x]]$end.pos,
                extr$segments[[x]]$Bf, extr$BAF[[x]])
        }, extr = sequenza_extract)
    if (pdf_out) {
        segs_i_clust <- file.path(out_path, paste(file_out_prefix,
            paste0("iter", i), "reclust.pdf", sep = "_"))
        pdf(segs_i_clust)
    }
    segs_clust <- cluster_segments(bf = segs_i$Bf, depth_ratio = segs_i$depth.ratio,
        init_clust = init_n_clust, progressbar = TRUE, iters = dp_iter,
        plots = pdf_out)
    if (pdf_out) {
        dev.off()
    }
    ...
    temp_extract <- sequenza.extract(seqz_file, breaks = seg_res,
        gc.stats = gc_stats, verbose = verbose, chromosome.list = sequenza_extract$chromosomes,
        ...)
    segs_fitting_ratio_i <- sapply(temp_extract$chromosomes,
        FUN = function(x, extr) {
            compare_bins(extr$segments[[x]]$start.pos, extr$segments[[x]]$end.pos,
                extr$segments[[x]]$depth.ratio, extr$ratio[[x]])
        }, extr = temp_extract)
    segs_fitting_baf_i <- sapply(temp_extract$chromosomes, FUN = function(x,
        extr) {
        compare_bins(extr$segments[[x]]$start.pos, extr$segments[[x]]$end.pos,
            extr$segments[[x]]$Bf, extr$BAF[[x]])
    }, extr = temp_extract)

    fit_table <- data.frame(chromosome = sequenza_extract$chromosomes,
        ratio_fit = segs_fitting_ratio[sequenza_extract$chromosomes],
        baf_fit = segs_fitting_baf[sequenza_extract$chromosomes],
        N = sapply(sequenza_extract$segments[sequenza_extract$chromosomes],
            nrow))

    refit_table <- data.frame(chromosome = temp_extract$chromosomes,
        ratio_fit = segs_fitting_ratio_i[temp_extract$chromosomes],
        baf_fit = segs_fitting_baf_i[temp_extract$chromosomes],
        N = sapply(temp_extract$segments[temp_extract$chromosomes],
            nrow))
