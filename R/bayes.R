...
baf.bayes <- function(
  ...
  baf.tab <- data.frame(
    Bf = Bf, ratio = log(depth.ratio), sd.Bf = sd.Bf, sd.ratio = sd.ratio,
  ...
  model.pts <- cbind(
    CNt = baf_types$CNt, A = baf_types$CNt - baf_types$B, B = baf_types$B,
    model_baf
  )
  ...
  bafs.L <- mapply(FUN = bayes.fit, rows.x, MoreArgs = list(
    mat = baf.tab, model.pts = model.pts,
    priors = priors, ratio.priority = ratio.priority
  ), SIMPLIFY = FALSE)
