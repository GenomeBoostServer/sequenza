...
baf.model.fit <- function(cellularity = seq(0.3, 1, by = 0.01),
    ploidy = seq(1, 7, by = 0.1), mc.cores = getOption("mc.cores",
        2L), ...) {
    # Create parameter grid
    result <- expand.grid(ploidy = ploidy, cellularity = cellularity,
        KEEP.OUT.ATTRS = FALSE)

    # Define fitting function for each parameter
    # combination
    fit.cp <- function(ii) {
    ...
    bayes.res <- pbapply::pblapply(X = 1:nrow(result), FUN = fit.cp,
        cl = cl)
...
mufreq.dbinom <- function(mufreq, mufreq.model, depth.t, seq.errors = 0.01,
    ...) {
    # Adjust zero mutation frequencies to sequence error
    # rate
...
depth.ratio.dbinom <- function(size, depth.ratio, depth.ratio.model,
    ...
    n.success <- round(size * (depth.ratio/(1 + depth.ratio)),
        0)
...
baf.bayes <- function(Bf, depth.ratio, cellularity, ploidy, avg.depth.ratio,
    ...
    CNt.min = 0, CNt.max = 7, CNn = 2, priors.table = data.frame(CN = CNt.min:CNt.max,
    ...
    model.pts <- cbind(CNt = baf_types$CNt, A = baf_types$CNt -
        baf_types$B, B = baf_types$B, model_baf)
    ...
    bayes.fit <- function(x, mat, model.pts, priors, ratio.priority) {
        ...
        if (!is.na(mat[x, ]$Bf) & !is.na(mat[x, ]$sd.Bf/sqrt(mat[x,
            ]$weight.Bf))) {
            score.b <- dt2(mean = mat[x, ]$Bf, sd = mat[x, ]$sd.Bf/sqrt(mat[x,
                ]$weight.Bf), x = test.baf, df = 5, log = TRUE)
    ...
    bafs.L <- mapply(FUN = bayes.fit, rows.x, MoreArgs = list(mat = baf.tab,
        model.pts = model.pts, priors = priors, ratio.priority = ratio.priority),
        SIMPLIFY = FALSE)
...
#' @rdname bayes
mufreq.bayes <- function(mufreq, depth.ratio, cellularity, ploidy,
    avg.depth.ratio, weight.mufreq = 100, weight.ratio = 100,
    CNt.min = 1, CNt.max = 7, CNn = 2, priors.table = data.frame(CN = CNt.min:CNt.max,
        value = 1)) {
    ...
    bayes.fit <- function(x, mat, model.pts, priors) {
        ...
        score.m <- mufreq.dbinom(mufreq = mat[x, ]$F, depth.t = mat[x,
            ]$weight.mufreq, test.mufrq)
