windowValues <- function(x, positions, chromosomes, window = 1e+06, overlap = 0,
    ...
    lapply(data.splitByChr, function(x) {
        ...
        window.quantiles <- sapply(x.window, quantile, probs = c(0.25, 0.75), na.rm = TRUE,
            names = FALSE)
...
windowBf <- function(Af, Bf, good.reads, positions, chromosomes, window = 1e+06,
    ...
    lapply(data.splitByChr, function(data.oneChr) {
        ...
        window.quantiles <- mapply(b_allele_freq, Af = A.window, Bf = B.window, good.reads = d.window,
            conf = conf)
