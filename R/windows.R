windowValues <- function(
    x, positions, chromosomes, window = 1e+06, overlap = 0, weight = rep.int(x = 1, times = length(x)),
    start.coord = 1
) {
    weight <- sqrt(weight)
    ...
    data.splitByChr <- split(
        data.frame(pos = positions, x = x, weight = weight),
        f = factor(chromosomes, levels = chr.ordered)
    )
    lapply(
        data.splitByChr, function(x) {
            ...
            idx.list <- lapply(
                1:nWindows, function(ii) ii +
                  (0:overlap)
            )
            x.window <- lapply(
                idx.list, function(idx) {
                  unlist(x.split[idx], use.names = FALSE)
                }
            )
            weight.window <- lapply(
                idx.list, function(idx) {
                  unlist(weight.split[idx], use.names = FALSE)
                }
            )
            window.means <- mapply(weighted.mean, x = x.window, w = weight.window)
            window.quantiles <- sapply(
                x.window, quantile, probs = c(0.25, 0.75),
                na.rm = TRUE, names = FALSE
            )
            window.counts <- sapply(x.window, length)
            data.frame(
                start = window.starts, end = window.ends, mean = window.means,
                q0 = window.quantiles[1, ], q1 = window.quantiles[2,
                  ], N = window.counts
            )
        }
    )
...
windowBf <- function(
    Af, Bf, good.reads, positions, chromosomes, window = 1e+06,
    ...
    chr.ordered <- unique(chromosomes)
    data.splitByChr <- split(
        data.frame(pos = positions, Bf, Af, good.reads),
        f = factor(chromosomes, levels = chr.ordered)
    )
    lapply(
        data.splitByChr, function(data.oneChr) {
            ...
            window.quantiles <- mapply(
                b_allele_freq, Af = A.window, Bf = B.window,
                good.reads = d.window, conf = conf
            )
            window.counts <- sapply(B.window, length)
            data.frame(
                start = window.starts, end = window.ends, mean = window.quantiles[2,
                  ], q0 = window.quantiles[1, ], q1 = window.quantiles[3,
                  ], N = window.counts
            )
        }
    )
