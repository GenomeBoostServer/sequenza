...
unfold_data <- function(x, f = do_get_gc, args_f = get_gc_defaults, stats = TRUE) {
    ...
    xyz_mats <- lapply(
        data_items, FUN = function(x, stat_obj, f, args) {
            f(stat_obj[, x], args)
...
data_fast_stats <- function(
    file, col_types = "c--dd----d----", buffer = 33554432, parallel = 2L,
    stats = TRUE, verbose = TRUE, col_sets = list(
        normal = c(2, 4),
        tumor = c(3, 4)
    ),
    ...
    parse_chunck <- function(x, col_types, sets = col_sets, f = f1, args_f = args_f1) {
        x <- read_tsv(
            ...
            col_types = col_types, col_names = FALSE, skip = 0,
            n_max = Inf, progress = FALSE
        )
        u_chr <- unique(x[, 1])
        n_chr <- table(x[, 1])
    ...
    res <- chunk.apply(
        input = con, FUN = function(x, col_types, sets, f, args) {
            parse_chunck(
                x, col_types = col_types, sets = sets, f = f,
                args
            )
...
get_baf_ratio <- function(
    ...
    all_ratios <- lapply(
        all_ratios, FUN = function(x) {
            sapply(
                sequenza:::splash_table(x),
                sum
            )
        }
    )
    ...
    n <- do.call(
        rbind, lapply(
            all_ratios, FUN = function(x, names_ratios) {
                res <- x[names_ratios]
    ...
    if (smooth == TRUE) {
        part <- gc_data_smooth(
            list(
                gc = as.numeric(names_baf),
                depth = as.numeric(names_ratios),
                n = n
            ),
...
baf_ratio_raster <- function(
    ...
    smooth = TRUE, grid_size = 100, scale.subset = 2.5, round_baf = 2,
    round_ratio = 1
) {
    ...
    do_ratio <- function(x, y, args) {
        ...
        d_r <- round(norm_tumor_depth/norm_normal_depth/0.5, digits = round_r) *
            0.5
        baf <- round(y$baf/0.5, digits = round_b) *
            0.5
    ...
    do_get_ratio_baf <- function(x, args) {
        ...
        get_baf_ratio(
            x, smooth = smooth, min_times = min_times, grid_size = grid_size,
            scale.subset = scale_subset
        )
    }

    data_fast_stats(
        file_name, col_types = "---dd--dcd----", stats = FALSE,
        verbose = verbose, col_sets = list(dr = c(normal = 1, tumor = 2, baf = 3, zyg = 4, gc = 5)),
        f1 = do_ratio, f2 = do_get_ratio_baf, args_f1 = ratio_args,
        args_f2 = get_gc_defaults, msg = "Collecting BAF/ratio information "
    )
}
...
rs_baf_ratio <- function(dens, n = 100, min_prob = 1) {
    ...
    res <- cbind(
        baf = sample_b, ratio = sample_r, l = df_b(sample_b)/max(p_bs) *
...
find_flex_points <- function(dens_list, f_threshold = 0.2) {
    flex_points_index <- which(
        diff(sign(diff(dens_list$z))) ==
            -2, arr.ind = TRUE
    )
    flex_points <- cbind(
        baf = dens_list$x[flex_points_index[, 1]], ratio = dens_list$y[flex_points_index[,
    ...
    for (i in 2:(m_col - 1)) {
        for (j in 2:(m_row - 1)) {
            ...
            diff_vect <- c(
                ij_val - x[left_cell_idx[1], left_cell_idx[2]],
                ij_val - x[right_cell_idx[1], right_cell_idx[2]],
                ij_val - x[up_cell_idx[1], up_cell_idx[2]], ij_val -
                  x[down_cell_idx[1], down_cell_idx[2]]
            )
...
find_local_max <- function(dens_list, f_threshold = 0.2) {
    ...
    flex_points <- cbind(
        baf = dens_list$x[max_points_index[, 1]], ratio = dens_list$y[max_points_index[,
...
smooth_matrix <- function(x, y, z) {
    ...
    H <- H * (m/n)^(2/d)
    kde(
        x = xy, w = weigths_xy/sum(weigths_xy) *
            m, H = H
    )
}
