...
unfold_data <- function(x, f = do_get_gc, args_f = get_gc_defaults,
    stats = TRUE) {
...
data_fast_stats <- function(file, col_types = "c--dd----d----",
    ...
    col_sets = list(normal = c(2, 4), tumor = c(3, 4)), f1 = gc_table,
    f2 = do_get_gc, args_f1 = list(), args_f2 = get_gc_defaults,
    msg = NULL) {
    ...
    parse_chunk <- function(x, col_types, sets = col_sets, f = f1,
        ...
        if (parallel > 1) {
            set_lists <- parallel::mclapply(sets, f, y = as.data.frame(chunk_data),
                args = args_f, mc.cores = min(parallel, length(sets)))
        } else {
            set_lists <- lapply(sets, f, y = as.data.frame(chunk_data),
                args = args_f)
        }
    ...
    results <- chunk.apply(input = con, FUN = parse_chunk, col_types = col_types,
...
get_baf_ratio <- function(baf_col, smooth = TRUE, min_times = 20,
    grid_size = 100, scale.subset = 1.5, ...) {
    ...
    names_ratios <- sort_char(unique(Reduce("c", lapply(all_ratios,
        names))))
    n <- do.call(rbind, lapply(all_ratios, FUN = function(x,
        names_ratios) {
        res <- x[names_ratios]
    ...
    n[is.na(n)] <- 0
    if (smooth == TRUE) {
        part <- gc_data_smooth(list(gc = as.numeric(names_baf),
            depth = as.numeric(names_ratios), n = n), min_times = min_times,
            n = grid_size, scale.subset = scale.subset, ...)
        names(part) <- c("x", "y", "z")
        list(raster = list(x = as.numeric(names_baf), y = as.numeric(names_ratios),
            z = n), smooth = part)
    } else {
        list(x = as.numeric(names_baf), y = as.numeric(names_ratios),
            z = n)
    }
}

baf_ratio_raster <- function(file_name, gc_normal, gc_tumor,
    verbose = TRUE, min_times = 20, smooth = TRUE, grid_size = 100,
    scale.subset = 2.5, round_baf = 2, round_ratio = 1) {
    ratio_args <- list(model_normal = gc_normal, model_tumor = gc_tumor,
        round_dr = round_ratio, round_bf = round_baf)

    get_gc_defaults <- list(smooth = smooth, min_times = min_times,
        grid_size = grid_size, scale_subset = scale.subset)
    ...
    do_ratio <- function(x, y, args) {
        ...
        d_r <- round(norm_tumor_depth/norm_normal_depth/0.5,
            digits = round_r) * 0.5
    ...
    data_fast_stats(file_name, col_types = "---dd--dcd----",
        stats = FALSE, verbose = verbose, col_sets = list(dr = c(normal = 1,
            tumor = 2, baf = 3, zyg = 4, gc = 5)), f1 = do_ratio,
        f2 = do_get_ratio_baf, args_f1 = ratio_args, args_f2 = get_gc_defaults,
...
find_flex_points <- function(dens_list, f_threshold = 0.2) {
    flex_points_index <- which(diff(sign(diff(dens_list$z))) ==
        -2, arr.ind = TRUE)
    flex_points <- cbind(baf = dens_list$x[flex_points_index[,
        1]], ratio = dens_list$y[flex_points_index[, 2]], l = dens_list$z[flex_points_index])
...
prepare_matrix_indices <- function(x) {
    list(rows = 2:(nrow(x) - 1), cols = 2:(ncol(x) - 1), neighbors = rbind(c(0,
        -1), c(0, 1), c(-1, 0), c(1, 0)))
}
    ...
    for (i in indices$cols) {
        for (j in indices$rows) {
            diffs <- sapply(1:4, function(k) {
                x[j, i] - x[j + indices$neighbors[k, 1], i +
                  indices$neighbors[k, 2]]
            })
    ...
    for (i in indices$cols) {
        for (j in indices$rows) {
            ...
            neighbors <- sapply(1:4, function(k) {
                x[j + indices$neighbors[k, 1], i + indices$neighbors[k,
                  2]]
            })
            res_m[j, i] <- as.integer(all(center - neighbors >=
                min_diff))
        }
    }
...
find_local_max <- function(dens_list, f_threshold = 0.2) {
    ...
    flex_points <- cbind(baf = dens_list$x[max_points_index[,
        1]], ratio = dens_list$y[max_points_index[, 2]], l = dens_list$z[max_points_index])
    flex_points <- flex_points[round(flex_points[, 1], 2) <=
        0.5, ]
    flex_points[, 3] <- flex_points[, 3]/max(dens_list$z)
    flex_points[flex_points[, 3] >= f_threshold, ]
}
