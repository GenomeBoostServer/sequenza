...
data_fast_stats <- function(file, col_types = "c--dd----d----", buffer = 33554432,
    parallel = 2L, stats = TRUE, verbose = TRUE, col_sets = list(normal = c(2, 4),
        tumor = c(3, 4)), f1 = gc_table, f2 = do_get_gc, args_f1 = list(), args_f2 = get_gc_defaults,
    msg = NULL) {
    con <- gzfile(file, "rb")
    ...
    res <- chunk.apply(input = con, FUN = function(x, col_types, sets, f, args) {
...
get_baf_ratio <- function(baf_col, smooth = TRUE, min_times = 20, grid_size = 100,
    scale.subset = 1.5, ...) {
    ...
    n[is.na(n)] <- 0
    if (smooth == TRUE) {
        part <- gc_data_smooth(list(gc = as.numeric(names_baf), depth = as.numeric(names_ratios),
...
baf_ratio_raster <- function(file_name, gc_normal, gc_tumor, verbose = TRUE, min_times = 20,
    smooth = TRUE, grid_size = 100, scale.subset = 2.5, round_baf = 2, round_ratio = 1) {

    ratio_args <- list(model_normal = gc_normal, model_tumor = gc_tumor, round_dr = round_ratio,
        round_bf = round_baf)

    get_gc_defaults <- list(smooth = smooth, min_times = min_times, grid_size = grid_size,
    ...
    do_get_ratio_baf <- function(x, args) {
        ...
        get_baf_ratio(x, smooth = smooth, min_times = min_times, grid_size = grid_size,
            scale.subset = scale_subset)
    }

    data_fast_stats(file_name, col_types = "---dd--dcd----", stats = FALSE, verbose = verbose,
...
find_flex_points <- function(dens_list, f_threshold = 0.2) {
    ...
    flex_points <- cbind(baf = dens_list$x[flex_points_index[, 1]], ratio = dens_list$y[flex_points_index[,
        2]], l = dens_list$z[flex_points_index])
...
mat_avg_peak <- function(x, min_diff = 0) {
    ...
    for (i in 2:(m_col - 1)) {
        for (j in 2:(m_row - 1)) {
            ...
            res_m[j, i] <- mean(c(ij_val - x[left_cell_idx[1], left_cell_idx[2]],
                ij_val - x[right_cell_idx[1], right_cell_idx[2]], ij_val - x[up_cell_idx[1],
                  up_cell_idx[2]], ij_val - x[down_cell_idx[1], down_cell_idx[2]]))
        }
    }
    res_m
}
    ...
    for (i in 2:(m_col - 1)) {
        for (j in 2:(m_row - 1)) {
            ...
            diff_vect <- c(ij_val - x[left_cell_idx[1], left_cell_idx[2]], ij_val -
                x[right_cell_idx[1], right_cell_idx[2]], ij_val - x[up_cell_idx[1],
                up_cell_idx[2]], ij_val - x[down_cell_idx[1], down_cell_idx[2]])
...
find_local_max <- function(dens_list, f_threshold = 0.2) {
    ...
    flex_points <- cbind(baf = dens_list$x[max_points_index[, 1]], ratio = dens_list$y[max_points_index[,
        2]], l = dens_list$z[max_points_index])
