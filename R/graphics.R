plotWindows <- function(seqz.window, m.lty = 1, m.lwd = 3, m.col = "black", q.bg = "lightblue",
  ...
  rect(
    xleft = seqz.window$start, ybottom = seqz.window$q0, xright = seqz.window$end,
    ytop = seqz.window$q1, col = q.bg, border = NA
  )
  segments(
    y0 = seqz.window$mean, x0 = seqz.window$start, x1 = seqz.window$end,
...
cp.plot <- function(cp.table, xlab = "Ploidy", ylab = "Cellularity", zlab = "Scaled rank LPP",
  ...
  colorgram(
    x = cp.table$ploidy, y = cp.table$cellularity, z = z, map = map, las = 1,
...
cp.plot.contours <- function(cp.table, likThresh = c(0.95), alternative = TRUE, col = palette(),
  ...
  contour(
    x = cp.table$ploidy, y = cp.table$cellularity, z = cp.table$lpp, levels = znormsort[n],
  ...
  if (!is.na(legend.pos)) {
    if (alternative == FALSE) {
      legend(legend.pos,
        legend = c(
          paste("C.R.", names(LikThresh), sep = " "),
          "Point estimate"
        ), col = c(col[1:length(LikThresh)], "black"), lty = c(rep(
          1,
          length(LikThresh)
        ), NA), pch = c(rep(NA, length(LikThresh)), pch),
        border = NA, bty = "n"
      )
    } else {
      legend(legend.pos, legend = c(
        paste("C.R.", names(LikThresh), sep = " "),
        "Point estimate", "Alternative solutions"
      ), col = c(
        col[1:length(LikThresh)],
        "black", "black"
      ), lty = c(rep(1, length(LikThresh)), NA, NA), pch = c(rep(
        NA,
        length(LikThresh)
      ), pch, alt.pch), border = NA, bty = "n")
    }
  }
  invisible(LikThresh)
}
...
chromosome.view <- function(baf.windows, ratio.windows, mut.tab = NULL, segments = NULL,
  ...
  if (is.null(segments)) {
    ...
    if ("CNt" %in% colnames(segments)) {
      if (length(c(cellularity, ploidy, avg.depth.ratio)) != 3) {
        ...
        CNt.min <- 0
        baf_types <- baf.types.matrix(
          CNt.min = CNt.min, CNt.max = CNt.max,
          CNn = 2
        )
        data.model$baf <- baf.model.points(
        ...
        mufreq_types <- mufreq.types.matrix(
          CNt.min = CNt.min, CNt.max = CNt.max,
          CNn = CNn
        )
        data.model$muf <- cbind(mufreq_types, mufreq.model.points(
  ...
  if (is.null(mut.tab)) {
    ...
    xlim <- c(min.x, max.x)
    par(
      mar = c(0, 4, 0, 10), oma = c(5, 0, 4, 0), mfcol = c(3, 1), xaxt = "n",
      xpd = TRUE
    )
    mutation.colors <- c(`A>C` = rgb(
      red = 0, green = 178, blue = 238, alpha = 120,
      maxColorValue = 255
    ), `T>G` = rgb(
      red = 0, green = 178, blue = 238, alpha = 120,
      maxColorValue = 255
    ), `A>G` = rgb(
      red = 255, green = 64, blue = 64, alpha = 120,
      maxColorValue = 255
    ), `T>C` = rgb(
      red = 255, green = 64, blue = 64, alpha = 120,
      maxColorValue = 255
    ), `A>T` = rgb(
      red = 34, green = 139, blue = 34, alpha = 120,
      maxColorValue = 255
    ), `T>A` = rgb(
      red = 34, green = 139, blue = 34, alpha = 120,
      maxColorValue = 255
    ), `C>A` = rgb(
      red = 139, green = 90, blue = 0, alpha = 120,
      maxColorValue = 255
    ), `G>T` = rgb(
      red = 139, green = 90, blue = 0, alpha = 120,
      maxColorValue = 255
    ), `C>G` = rgb(
      red = 127, green = 0, blue = 255, alpha = 120,
      maxColorValue = 255
    ), `G>C` = rgb(
      red = 127, green = 0, blue = 255, alpha = 120,
      maxColorValue = 255
    ), `C>T` = rgb(
      red = 255, green = 215, blue = 0, alpha = 120,
      maxColorValue = 255
    ), `G>A` = rgb(
      red = 255, green = 215, blue = 0, alpha = 120,
      maxColorValue = 255
    ), Ins = rgb(
      red = 47, green = 79, blue = 79, alpha = 120,
      maxColorValue = 255
    ), Dels = rgb(
    ...
    plot(
      x = mut.tab$position, y = mut.tab$F, ylab = "Mutant allele frequency",
    ...
    legend(
      y = "center", x = "right", legend = labels, inset = legend.inset,
    ...
    if (!is.null(segments)) {
      ...
      if (!is.null(data.model)) {
        for (i in 1:nrow(segments)) {
          segments(
            x0 = segments$start.pos[i], x1 = segments$end.pos[i],
  ...
  if (!is.null(segments)) {
    ...
    plotWindows(baf.windows, ylab = "B allele frequency", xlim = xlim, ylim = c(
      0,
      0.5
    ), las = 1, n.min = min.N.baf, add = TRUE)
    if (vlines) {
      abline(v = segments$end.pos, lwd = 1, lty = 2)
    }
    segments(
      x0 = segments$start.pos, y0 = segments$Bf, x1 = segments$end.pos,
    ...
    plotWindows(baf.windows, ylab = "B allele frequency", xlim = xlim, ylim = c(
      0,
      0.5
    ), las = 1, n.min = min.N.baf)
  }
  plotWindows(ratio.windows,
    ylab = "Depth ratio", las = 1, n.min = min.N.ratio,
    ylim = c(0, 2.5)
  )
  if (!is.null(segments)) {
    if (vlines) {
    ...
    segments(
      x0 = segments$start.pos, y0 = segments$depth.ratio, x1 = segments$end.pos,
      y1 = segments$depth.ratio, col = "red", lwd = 3
    )
    if (!is.null(data.model)) {
      ratios.theoric <- unique(data.model$muf[, c("CNt", "depth.ratio")])
      segments(
        x0 = rep(min(segments$start.pos, na.rm = TRUE), times = nrow(ratios.theoric)),
      ...
      axis(
        labels = as.character(ratios.theoric$CNt), side = 4, line = 0, las = 1,
  ...
  par(xaxt = "s")
  axis(
    labels = as.character(round(
      seq(xlim[1] / 1e+06, xlim[2] / 1e+06, by = x.chr.space),
      0
    )), side = 1, line = 0, at = seq(xlim[1], xlim[2], by = 1e+06 * x.chr.space),
...
genome.view <- function(seg.cn, info.type = "AB", ...) {
  ...
  seg.list <- split(x = seg.cn[, c(
    "chromosome", "start.pos", "end.pos", "A", "B",
    "CNt"
  )], f = seg.cn$chromosome)
  ...
  if (info.type == "AB") {
    ...
    plot(
      x = c(min(abs.segments$abs.start), max(abs.segments$abs.end)), y = c(
        -0.1,
        (max_A + 0.1)
      ), type = "n", ylab = "Copy number", xlab = "Position (Mb)",
    ...
    plot(
      x = c(min(abs.segments$abs.start), max(abs.segments$abs.end)), y = c(
        min_CNt,
        max_CNt
      ), type = "n", ylab = "Copy number", xlab = "Position (Mb)", xaxt = "n",
      yaxt = "n", xaxs = "i", ...
    )
    axis(
      labels = min_CNt:max_CNt, at = min_CNt:max_CNt, side = 2, line = 0,
      las = 1
    )
    segments(
      x0 = abs.segments$abs.start, x1 = abs.segments$abs.end, y0 = abs.segments$CNt,
  ...
  for (i in 1:length(abs.list)) {
    ...
    mtext(chr.order[i], side = 3, line = 0, at = sum(
      abs.list[[i]]$abs.start[1],
      abs.list[[i]]$abs.end[max.pos]
    ) / 2)
  }
  axis(
    labels = as.character(round(seq(abs.list[[1]]$start.pos[1] / 1e+06, abs.list[[1]]$end.pos[nrow(abs.list[[1]])] / 1e+06,
      by = 50
...
plotRawGenome <- function(sequenza.extract, cellularity, ploidy, CNt.max = 7, main = "",
  ...
  if (mirror.BAF) {
    ...
    plot(
      x = c(min(max.end), max(max.end)), y = c(0, 1), main = main, xlab = NA,
      ylab = "Allele frequency", type = "n", las = 1, xaxs = "i", yaxs = "i",
      xaxt = "n"
    )
    plotWindows(
      seqz.window = do.call(rbind, AAF.new), q.bg = "lightblue", m.col = "black",
      add = T
    )
    segments(
      x0 = segs.new$start.pos, x1 = segs.new$end.pos, y0 = 1 - (segs.new$Bf),
      y1 = 1 - (segs.new$Bf), col = "red", lwd = 2, lend = 1
    )
  } else {
    plot(
      x = c(min(max.end), max(max.end)), y = c(0, 0.5), main = main, xlab = NA,
      ylab = "B allele frequency", type = "n", las = 1, xaxs = "i", yaxs = "i",
      xaxt = "n"
    )
  }
  plotWindows(
    seqz.window = do.call(rbind, BAF.new), q.bg = "lightblue", m.col = "black",
    add = T
  )
  segments(
    x0 = segs.new$start.pos, x1 = segs.new$end.pos, y0 = segs.new$Bf, y1 = segs.new$Bf,
    col = "red", lwd = 2, lend = 1
  )
  abline(v = max.end, lty = 1)
  plot(
    x = c(min(max.end), max(max.end)), y = c(0, 2.5), main = "", xlab = NA,
    ylab = "Depth ratio", type = "n", las = 1, xaxs = "i", yaxs = "i", xaxt = "n"
  )
  plotWindows(
    seqz.window = do.call(rbind, ratio.new), q.bg = "lightblue", m.col = "black",
    add = T
  )
  segments(
    x0 = segs.new$start.pos, x1 = segs.new$end.pos, y0 = (segs.new$depth.ratio),
  ...
  if (!missing(ploidy) & !missing(cellularity)) {
    ...
    depth.ratios <- baf.model.points(
      cellularity = cellularity, ploidy = ploidy,
      avg.depth.ratio = avg.depth.ratio, baf_types = types
    )[, "depth.ratio"]
    depth.ratios <- unique(data.frame(CNt = types$CNt, ratio = depth.ratios))
    abline(h = depth.ratios$ratio, lty = 2)
    axis(
      labels = as.character(depth.ratios$CNt), side = 4, line = 0, las = 1,
...
baf.model.view <- function(cellularity, ploidy, segs, BAF.space = seq(
  ...
  both.space <- baf.bayes(
    Bf = test.values$Bf, CNt.max = CNt.max, CNt.min = 0,
  ...
  suppressWarnings(colorgram(x, y, z,
    key = NA, nz = 1000, xlab = "B allele frequency",
    ylab = "Depth ratio", main = paste("cellularity:", cellularity, "ploidy:",
      ploidy, "sd.BAF:", round(s.b, 2),
      sep = " "
    ), map = makecmap(z, breaks = unique(quantile(
      z,
      seq(0.25, 1, 1e-04)
  ...
  points(
    x = segs$Bf[!s.big], y = segs$depth.ratio[!s.big], pch = ".", cex = 1,
    col = segs$col[!s.big]
  )
}
