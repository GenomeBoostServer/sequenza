plotWindows <- function(seqz.window, m.lty = 1, m.lwd = 3, m.col = "black", q.bg = "lightblue",
    ...
    rect(xleft = seqz.window$start, ybottom = seqz.window$q0, xright = seqz.window$end,
        ytop = seqz.window$q1, col = q.bg, border = NA)
...
gc.summary.plot <- function(gc_list, mean.col = 1, median.col = 2, scale.subset = 1.5,
    ...
    legend("topright", c("Mean depth", "Median depth"), col = c(mean.col, median.col),
        bg = "white", lty = 1, lwd = 3)
}
...
cp.plot <- function(cp.table, xlab = "Ploidy", ylab = "Cellularity", zlab = "Scaled rank LPP",
    ...
    colorgram(x = cp.table$ploidy, y = cp.table$cellularity, z = z, map = map, las = 1,
...
cp.plot.contours <- function(cp.table, likThresh = c(0.95), alternative = TRUE, col = palette(),
    ...
    contour(x = cp.table$ploidy, y = cp.table$cellularity, z = cp.table$lpp, levels = znormsort[n],
        col = col, drawlabels = FALSE, xlab = "Ploidy", ylab = "Cellularity", ...)
    max.xy <- which(cp.table$lpp == max(cp.table$lpp), arr.ind = TRUE)
    points(x = cp.table$ploidy[max.xy[, "row"]], y = cp.table$cellularity[max.xy[,
        "col"]], pch = pch)
    if (alternative == TRUE) {
    ...
    if (!is.na(legend.pos)) {
        if (alternative == FALSE) {
            legend(legend.pos, legend = c(paste("C.R.", names(LikThresh), sep = " "),
                "Point estimate"), col = c(col[1:length(LikThresh)], "black"), lty = c(rep(1,
                length(LikThresh)), NA), pch = c(rep(NA, length(LikThresh)), pch),
                border = NA, bty = "n")
        } else {
            legend(legend.pos, legend = c(paste("C.R.", names(LikThresh), sep = " "),
                "Point estimate", "Alternative solutions"), col = c(col[1:length(LikThresh)],
                "black", "black"), lty = c(rep(1, length(LikThresh)), NA, NA), pch = c(rep(NA,
                length(LikThresh)), pch, alt.pch), border = NA, bty = "n")
        }
    }
    invisible(LikThresh)
}
...
chromosome.view <- function(baf.windows, ratio.windows, mut.tab = NULL, segments = NULL,
    ...
    if (is.null(segments)) {
        ...
        if ("CNt" %in% colnames(segments)) {
            if (length(c(cellularity, ploidy, avg.depth.ratio)) != 3) {
                ...
                CNt.min <- 0
                baf_types <- baf.types.matrix(CNt.min = CNt.min, CNt.max = CNt.max,
                  CNn = 2)
                data.model$baf <- baf.model.points(cellularity = cellularity, ploidy = ploidy,
                  baf_types = baf_types, avg.depth.ratio = avg.depth.ratio)
                data.model$baf <- data.frame(CNt = baf_types$CNt, A = baf_types$CNt -
                  baf_types$B, B = baf_types$B, data.model$baf)
                mufreq_types <- mufreq.types.matrix(CNt.min = CNt.min, CNt.max = CNt.max,
                  CNn = CNn)
                data.model$muf <- cbind(mufreq_types, mufreq.model.points(cellularity = cellularity,
                  ploidy = ploidy, mufreq_types = mufreq_types, avg.depth.ratio = avg.depth.ratio))
            }
        } else {
            data.model <- NULL
        }
    ...
    if (is.null(mut.tab)) {
        ...
        max.x <- max(c(max(baf.windows$end), max(ratio.windows$end), max(mut.tab$position)),
            na.rm = TRUE)
        xlim <- c(min.x, max.x)
        par(mar = c(0, 4, 0, 10), oma = c(5, 0, 4, 0), mfcol = c(3, 1), xaxt = "n",
            xpd = TRUE)
        mutation.colors <- c(`A>C` = rgb(red = 0, green = 178, blue = 238, alpha = 120,
            maxColorValue = 255), `T>G` = rgb(red = 0, green = 178, blue = 238, alpha = 120,
            maxColorValue = 255), `A>G` = rgb(red = 255, green = 64, blue = 64, alpha = 120,
            maxColorValue = 255), `T>C` = rgb(red = 255, green = 64, blue = 64, alpha = 120,
            maxColorValue = 255), `A>T` = rgb(red = 34, green = 139, blue = 34, alpha = 120,
            maxColorValue = 255), `T>A` = rgb(red = 34, green = 139, blue = 34, alpha = 120,
            maxColorValue = 255), `C>A` = rgb(red = 139, green = 90, blue = 0, alpha = 120,
            maxColorValue = 255), `G>T` = rgb(red = 139, green = 90, blue = 0, alpha = 120,
            maxColorValue = 255), `C>G` = rgb(red = 127, green = 0, blue = 255, alpha = 120,
            maxColorValue = 255), `G>C` = rgb(red = 127, green = 0, blue = 255, alpha = 120,
            maxColorValue = 255), `C>T` = rgb(red = 255, green = 215, blue = 0, alpha = 120,
            maxColorValue = 255), `G>A` = rgb(red = 255, green = 215, blue = 0, alpha = 120,
            maxColorValue = 255), Ins = rgb(red = 47, green = 79, blue = 79, alpha = 120,
            maxColorValue = 255), Dels = rgb(red = 169, green = 169, blue = 169,
            alpha = 120, maxColorValue = 255))
        mutation_types <- unlist(lapply(strsplit(as.character(mut.tab$mutation),
        ...
        plot(x = mut.tab$position, y = mut.tab$F, ylab = "Mutant allele frequency",
            las = 1, pch = 19, col = c(mutation.colors, `NA` = NA)[as.character(mutation_types)],
            ylim = c(min(mut.tab$F, na.rm = TRUE), 1), xlim = xlim)
        unique.colors <- unique(mutation.colors)
        labels <- sapply(unique.colors, function(a) {
            paste(names(mutation.colors)[mutation.colors == a], collapse = ", ")
        })
        legend(y = "center", x = "right", legend = labels, inset = legend.inset,
            pch = 19, col = unique.colors, pt.bg = unique.colors, border = NA, bty = "n")
        if (!is.null(segments)) {
            if (vlines) {
            ...
            if (!is.null(data.model)) {
                for (i in 1:nrow(segments)) {
                  segments(x0 = segments$start.pos[i], x1 = segments$end.pos[i],
                    y0 = unique(data.model$muf$mufreqs[data.model$muf$CNt == segments$CNt[i]]),
    ...
    if (!is.null(segments)) {
        plot(ylab = "B allele frequency", type = "n", x = xlim, y = c(0, 0.5), las = 1)
        plotWindows(baf.windows, ylab = "B allele frequency", xlim = xlim, ylim = c(0,
            0.5), las = 1, n.min = min.N.baf, add = TRUE)
        if (vlines) {
            abline(v = segments$end.pos, lwd = 1, lty = 2)
        }
        segments(x0 = segments$start.pos, y0 = segments$Bf, x1 = segments$end.pos,
            y1 = segments$Bf, col = "red", lwd = 3)
        if (!is.null(data.model)) {
            for (i in 1:nrow(segments)) {
                segments(x0 = segments$start.pos[i], x1 = segments$end.pos[i], y0 = unique(data.model$baf$BAF[data.model$baf$CNt ==
                  segments$CNt[i]]), lwd = model.lwd, lty = model.lty, col = model.col)
            }
        }
    } else {
        plotWindows(baf.windows, ylab = "B allele frequency", xlim = xlim, ylim = c(0,
            0.5), las = 1, n.min = min.N.baf)
    }
    plotWindows(ratio.windows, ylab = "Depth ratio", las = 1, n.min = min.N.ratio,
        ylim = c(0, 2.5))
    if (!is.null(segments)) {
        if (vlines) {
            abline(v = segments$end.pos, lwd = 1, lty = 2)
        }
        segments(x0 = segments$start.pos, y0 = segments$depth.ratio, x1 = segments$end.pos,
            y1 = segments$depth.ratio, col = "red", lwd = 3)
        if (!is.null(data.model)) {
            ...
            segments(x0 = rep(min(segments$start.pos, na.rm = TRUE), times = nrow(ratios.theoric)),
                ...
                y0 = ratios.theoric$depth.ratio, lwd = model.lwd, lty = model.lty,
                col = model.col)
    ...
    par(xaxt = "s")
    axis(labels = as.character(round(seq(xlim[1]/1e+06, xlim[2]/1e+06, by = x.chr.space),
        0)), side = 1, line = 0, at = seq(xlim[1], xlim[2], by = 1e+06 * x.chr.space),
        outer = FALSE, cex = par("cex.axis") * par("cex"))
    mtext("Position (Mb)", side = 1, line = 3, outer = FALSE, cex = par("cex.lab") *
        par("cex"))
...
genome.view <- function(seg.cn, info.type = "AB", ...) {
    ...
    seg.list <- split(x = seg.cn[, c("chromosome", "start.pos", "end.pos", "A", "B",
        "CNt")], f = seg.cn$chromosome)
    ...
    if (info.type == "AB") {
        ...
        plot(x = c(min(abs.segments$abs.start), max(abs.segments$abs.end)), y = c(-0.1,
            (max_A + 0.1)), type = "n", ylab = "Copy number", xlab = "Position (Mb)",
            xaxt = "n", yaxt = "n", xaxs = "i", ...)
        axis(labels = 0:max_A, at = 0:max_A, side = 2, line = 0, las = 1)
        segments(x0 = abs.segments$abs.start, x1 = abs.segments$abs.end, y0 = (abs.segments$B -
            0.1), y1 = (abs.segments$B - 0.1), col = "blue", lwd = 5, lend = 1)
        segments(x0 = abs.segments$abs.start, x1 = abs.segments$abs.end, y0 = (abs.segments$A +
            0.1), y1 = (abs.segments$A + 0.1), col = "red", lwd = 5, lend = 1)
    } else {
        min_CNt <- min(abs.segments$CNt, na.rm = TRUE)
        max_CNt <- max(abs.segments$CNt, na.rm = TRUE)
        plot(x = c(min(abs.segments$abs.start), max(abs.segments$abs.end)), y = c(min_CNt,
            max_CNt), type = "n", ylab = "Copy number", xlab = "Position (Mb)", xaxt = "n",
            yaxt = "n", xaxs = "i", ...)
        axis(labels = min_CNt:max_CNt, at = min_CNt:max_CNt, side = 2, line = 0,
            las = 1)
        segments(x0 = abs.segments$abs.start, x1 = abs.segments$abs.end, y0 = abs.segments$CNt,
            y1 = abs.segments$CNt, col = "red", lwd = 5, lend = 1)
    }
    ...
    axis(labels = as.character(round(seq(abs.list[[1]]$start.pos[1]/1e+06, abs.list[[1]]$end.pos[nrow(abs.list[[1]])]/1e+06,
        by = 50), 0)), at = seq(abs.list[[1]]$abs.start[1], abs.list[[1]]$abs.end[nrow(abs.list[[1]])],
        by = 5e+07), outer = FALSE, cex = par("cex.axis") * par("cex"), side = 1,
        line = 1)
}
...
plotRawGenome <- function(sequenza.extract, cellularity, ploidy, CNt.max = 7, main = "",
    ...
    if (mirror.BAF) {
        ...
        plot(x = c(min(max.end), max(max.end)), y = c(0, 1), main = main, xlab = NA,
            ylab = "Allele frequency", type = "n", las = 1, xaxs = "i", yaxs = "i",
            xaxt = "n")
        plotWindows(seqz.window = do.call(rbind, AAF.new), q.bg = "lightblue", m.col = "black",
            add = T)
        segments(x0 = segs.new$start.pos, x1 = segs.new$end.pos, y0 = 1 - (segs.new$Bf),
            y1 = 1 - (segs.new$Bf), col = "red", lwd = 2, lend = 1)
    } else {
        plot(x = c(min(max.end), max(max.end)), y = c(0, 0.5), main = main, xlab = NA,
            ylab = "B allele frequency", type = "n", las = 1, xaxs = "i", yaxs = "i",
            xaxt = "n")
    }
    plotWindows(seqz.window = do.call(rbind, BAF.new), q.bg = "lightblue", m.col = "black",
        add = T)
    segments(x0 = segs.new$start.pos, x1 = segs.new$end.pos, y0 = segs.new$Bf, y1 = segs.new$Bf,
    ...
    plotWindows(seqz.window = do.call(rbind, ratio.new), q.bg = "lightblue", m.col = "black",
        add = T)
    segments(x0 = segs.new$start.pos, x1 = segs.new$end.pos, y0 = (segs.new$depth.ratio),
        y1 = (segs.new$depth.ratio), col = "red", lwd = 2, lend = 1)
    if (!missing(ploidy) & !missing(cellularity)) {
        types <- baf.types.matrix(CNt.min = 0, CNt.max = CNt.max, CNn = 2)
        depth.ratios <- baf.model.points(cellularity = cellularity, ploidy = ploidy,
            avg.depth.ratio = avg.depth.ratio, baf_types = types)[, "depth.ratio"]
...
baf.model.view <- function(cellularity, ploidy, segs, BAF.space = seq(0.001, 0.5,
    0.005), ratio.space = seq(0.01, 2.5, 0.05), avg.depth.ratio = 1, CNt.max = 7,
    ...
    test.values <- expand.grid(Bf = BAF.space, ratio = ratio.space, KEEP.OUT.ATTRS = FALSE)
    both.space <- baf.bayes(Bf = test.values$Bf, CNt.max = CNt.max, CNt.min = 0,
        depth.ratio = test.values$ratio, cellularity = cellularity, ploidy = ploidy,
        avg.depth.ratio = avg.depth.ratio, sd.Bf = s.b, weight.Bf = 10, sd.ratio = s.r,
        weight.ratio = 10, ratio.priority = F, CNn = 2)
    ...
    mpts <- cbind(t, baf.model.points(cellularity = cellularity, ploidy = ploidy,
        baf_types = t, avg.depth.ratio = avg.depth.ratio))
    mpts <- unique(mpts[, c("CNt", "depth.ratio")])
    par(mar = c(5.1, 4.1, 4.1, 4.1))
    rev.heat <- function(...) {
        rev(heat.colors(...))
    }
    suppressWarnings(colorgram(x, y, z, key = NA, nz = 1000, xlab = "B allele frequency",
        ylab = "Depth ratio", main = paste("cellularity:", cellularity, "ploidy:",
            ploidy, "sd.BAF:", round(s.b, 2), sep = " "), map = makecmap(z, breaks = unique(quantile(z,
            seq(0.25, 1, 1e-04))), right = TRUE, n = 1000, colFn = rev.heat), outlier = "white",
        las = 1, xlim = c(0, 0.5)))
...
plot_peaks_win <- function(rank_wins_peak, ...) {
    ...
    par(mfrow = c(1, 2))
    select_color <- "green"
    point_sizes <- setNames(rep(as.numeric(par()["cex"]), times = nrow(rank_wins_peak$peak_win)),
    ...
    range_fits <- range(c(rank_wins_peak$peak_win$baf_fit, rank_wins_peak$peak_win$ratio_fit),
        na.rm = TRUE)
    plot(x = rank_wins_peak$peak_win$peak_win, y = rank_wins_peak$peak_win$baf_fit,
        ...
        cex = point_sizes, ...)
    lines(x = rank_wins_peak$peak_win$peak_win, y = rank_wins_peak$peak_win$ratio_fit,
        type = "b", pch = 21, bg = win_colors, col = "red", cex = point_sizes)
    legend("topright", c("baf fit", "depth ratio fit", "selected"), lty = c(1, 1,
        NA), pch = c(NA, NA, 21), col = c("blue", "red", "black"), pt.bg = c(NA,
        NA, select_color), pt.cex = c(NA, NA, point_sizes[selected_index]), bty = "n")
