plotWindows <- function(seqz.window, m.lty = 1, m.lwd = 3, m.col = "black",
    q.bg = "lightblue", log2.plot = FALSE, n.min = 1, xlim, ylim,
    add = FALSE, ...) {
    if (log2.plot) {
        seqz.window[, c(3, 4, 5)] <- log2(seqz.window[, c(3,
            4, 5)])
    }
    if (!add) {
        if (missing(xlim)) {
        ...
        if (missing(ylim)) {
            ylim <- c(min(seqz.window$q0, na.rm = TRUE), max(seqz.window$q1,
                na.rm = TRUE))
        }
...
gc.summary.plot <- function(gc_list, mean.col = 1, median.col = 2,
    ...
    legend("topright", c("Mean depth", "Median depth"), col = c(mean.col,
        median.col), bg = "white", lty = 1, lwd = 3)
}

cp.plot <- function(cp.table, xlab = "Ploidy", ylab = "Cellularity",
    zlab = "Scaled rank LPP", colFn = colorRampPalette(c("white",
        "lightblue")), ...) {
    z <- matrix(rank(cp.table$lpp), nrow = nrow(cp.table$lpp))/length(cp.table$lpp)
    map <- makecmap(c(0, 1), colFn = colFn, include.lowest = TRUE)
    colorgram(x = cp.table$ploidy, y = cp.table$cellularity,
        z = z, map = map, las = 1, xlab = xlab, ylab = ylab,
        zlab = zlab, ...)
}
...
cp.plot.contours <- function(cp.table, likThresh = c(0.95), alternative = TRUE,
    ...
    n <- sapply(likThresh, function(x) sum(znormcumLik < x) +
        1)
    ...
    if (!is.na(legend.pos)) {
        if (alternative == FALSE) {
            legend(legend.pos, legend = c(paste("C.R.", names(LikThresh),
                sep = " "), "Point estimate"), col = c(col[1:length(LikThresh)],
                "black"), lty = c(rep(1, length(LikThresh)),
                NA), pch = c(rep(NA, length(LikThresh)), pch),
                border = NA, bty = "n")
        } else {
            legend(legend.pos, legend = c(paste("C.R.", names(LikThresh),
                sep = " "), "Point estimate", "Alternative solutions"),
                col = c(col[1:length(LikThresh)], "black", "black"),
                lty = c(rep(1, length(LikThresh)), NA, NA), pch = c(rep(NA,
                  length(LikThresh)), pch, alt.pch), border = NA,
                bty = "n")
        }
    }
    invisible(LikThresh)
...
chromosome.view <- function(baf.windows, ratio.windows, mut.tab = NULL,
        ...
        if ("CNt" %in% colnames(segments)) {
            if (length(c(cellularity, ploidy, avg.depth.ratio)) !=
                ...
                data.model$baf <- baf.model.points(cellularity = cellularity,
                  ploidy = ploidy, baf_types = baf_types, avg.depth.ratio = avg.depth.ratio)
    ...
    if (is.null(mut.tab)) {
        par(mar = c(0, 4, 0, 3), oma = c(5, 0, 4, 0), mfcol = c(2,
            1), xaxt = "n")
        ...
        xlim <- c(min.x, max.x)
        par(mar = c(0, 4, 0, 10), oma = c(5, 0, 4, 0), mfcol = c(3,
            1), xaxt = "n", xpd = TRUE)
        mutation.colors <- c(`A>C` = rgb(red = 0, green = 178,
            blue = 238, alpha = 120, maxColorValue = 255), `T>G` = rgb(red = 0,
            green = 178, blue = 238, alpha = 120, maxColorValue = 255),
            `A>G` = rgb(red = 255, green = 64, blue = 64, alpha = 120,
                maxColorValue = 255), `T>C` = rgb(red = 255,
                green = 64, blue = 64, alpha = 120, maxColorValue = 255),
            `A>T` = rgb(red = 34, green = 139, blue = 34, alpha = 120,
                maxColorValue = 255), `T>A` = rgb(red = 34, green = 139,
                blue = 34, alpha = 120, maxColorValue = 255),
            `C>A` = rgb(red = 139, green = 90, blue = 0, alpha = 120,
                maxColorValue = 255), `G>T` = rgb(red = 139,
                green = 90, blue = 0, alpha = 120, maxColorValue = 255),
            `C>G` = rgb(red = 127, green = 0, blue = 255, alpha = 120,
                maxColorValue = 255), `G>C` = rgb(red = 127,
                green = 0, blue = 255, alpha = 120, maxColorValue = 255),
            `C>T` = rgb(red = 255, green = 215, blue = 0, alpha = 120,
                maxColorValue = 255), `G>A` = rgb(red = 255,
                green = 215, blue = 0, alpha = 120, maxColorValue = 255),
            Ins = rgb(red = 47, green = 79, blue = 79, alpha = 120,
                maxColorValue = 255), Dels = rgb(red = 169, green = 169,
                blue = 169, alpha = 120, maxColorValue = 255))
            ...
            if (!is.null(data.model)) {
                for (i in 1:nrow(segments)) {
                  segments(x0 = segments$start.pos[i], x1 = segments$end.pos[i],
                    y0 = unique(data.model$muf$mufreqs[data.model$muf$CNt ==
                      segments$CNt[i]]), lwd = model.lwd, lty = model.lty,
                    col = model.col)
                }
            }
        }
    ...
    if (!is.null(segments)) {
        plot(ylab = "B allele frequency", type = "n", x = xlim,
            y = c(0, 0.5), las = 1)
        ...
        if (!is.null(data.model)) {
            for (i in 1:nrow(segments)) {
                segments(x0 = segments$start.pos[i], x1 = segments$end.pos[i],
                  y0 = unique(data.model$baf$BAF[data.model$baf$CNt ==
                    segments$CNt[i]]), lwd = model.lwd, lty = model.lty,
                  col = model.col)
            }
        }
    ...
    plotWindows(ratio.windows, ylab = "Depth ratio", las = 1,
        n.min = min.N.ratio, ylim = c(0, 2.5))
    if (!is.null(segments)) {
        if (vlines) {
            abline(v = segments$end.pos, lwd = 1, lty = 2)
        }
        ...
        if (!is.null(data.model)) {
            ...
            segments(x0 = rep(min(segments$start.pos, na.rm = TRUE),
                times = nrow(ratios.theoric)), x1 = rep(max(segments$end.pos,
                na.rm = TRUE), times = nrow(ratios.theoric)),
    ...
    par(xaxt = "s")
    axis(labels = as.character(round(seq(xlim[1]/1e+06, xlim[2]/1e+06,
        by = x.chr.space), 0)), side = 1, line = 0, at = seq(xlim[1],
        xlim[2], by = 1e+06 * x.chr.space), outer = FALSE, cex = par("cex.axis") *
        par("cex"))
    mtext("Position (Mb)", side = 1, line = 3, outer = FALSE,
        cex = par("cex.lab") * par("cex"))
    mtext(main, 3, outer = TRUE, cex = par("cex.main") * par("cex"),
        line = 2)
}
...
genome.view <- function(seg.cn, info.type = "AB", ...) {
    ...
    axis(labels = as.character(round(seq(abs.list[[1]]$start.pos[1]/1e+06,
        abs.list[[1]]$end.pos[nrow(abs.list[[1]])]/1e+06, by = 50),
        0)), at = seq(abs.list[[1]]$abs.start[1], abs.list[[1]]$abs.end[nrow(abs.list[[1]])],
        by = 5e+07), outer = FALSE, cex = par("cex.axis") * par("cex"),
        side = 1, line = 1)
}
...
plotRawGenome <- function(sequenza.extract, cellularity, ploidy,
    ...
    plotWindows(seqz.window = do.call(rbind, BAF.new), q.bg = "lightblue",
        m.col = "black", add = T)
    segments(x0 = segs.new$start.pos, x1 = segs.new$end.pos,
        y0 = segs.new$Bf, y1 = segs.new$Bf, col = "red", lwd = 2,
        lend = 1)
...
baf.model.view <- function(cellularity, ploidy, segs, BAF.space = seq(0.001,
    0.5, 0.005), ratio.space = seq(0.01, 2.5, 0.05), avg.depth.ratio = 1,
    CNt.max = 7, segment.filter = 3e+06, col = "black") {
    ...
    both.space <- baf.bayes(Bf = test.values$Bf, CNt.max = CNt.max,
        CNt.min = 0, depth.ratio = test.values$ratio, cellularity = cellularity,
    ...
    z <- tapply(both.space$LPP, list(test.values$Bf, test.values$ratio),
        mean)
    ...
    suppressWarnings(colorgram(x, y, z, key = NA, nz = 1000,
        xlab = "B allele frequency", ylab = "Depth ratio", main = paste("cellularity:",
            cellularity, "ploidy:", ploidy, "sd.BAF:", round(s.b,
                2), sep = " "), map = makecmap(z, breaks = unique(quantile(z,
            seq(0.25, 1, 1e-04))), right = TRUE, n = 1000, colFn = rev.heat),
        outlier = "white", las = 1, xlim = c(0, 0.5)))
    axis(side = 4, at = mpts$depth.ratio, labels = mpts$CNt,
        las = 1)
    mtext(text = "Copy number", side = 4, line = 2)
    segs$col <- col
    points(x = segs$Bf[s.big], y = segs$depth.ratio[s.big], pch = 1,
        cex = 1, col = segs$col[s.big])
    points(x = segs$Bf[!s.big], y = segs$depth.ratio[!s.big],
        pch = ".", cex = 1, col = segs$col[!s.big])
}
...
plot_peaks_win <- function(rank_wins_peak, ...) {
    ...
    win_colors <- setNames(rep("gray", times = nrow(rank_wins_peak$peak_win)),
        as.character(rank_wins_peak$peak_win$peak_win))
    selected_index <- which(rank_wins_peak$peak_win$peak_win ==
    ...
    legend("topright", c("baf fit", "depth ratio fit", "selected"),
        lty = c(1, 1, NA), pch = c(NA, NA, 21), col = c("blue",
            "red", "black"), pt.bg = c(NA, NA, select_color),
    ...
    legend("topright", c("selected window size"), fill = select_color,
        bty = "n")
}
